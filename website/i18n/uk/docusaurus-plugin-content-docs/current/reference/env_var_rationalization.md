---
title: "Обґрунтування використання змінних середовища."
description: "Повний план класифікації та об’єднання змінних середовища виконання SIMA."
sidebar_position: 3
---

# Оптимізація змінних середовища

Середовище виконання. `SIMA_*` змінні, на які посилається код, класифіковано на чотири категорії, щоб зменшити надмірну кількість параметрів конфігурації.
Кожна змінна з’являється лише один раз у наведених нижче списках.

## Усього

- Знайдено 148 змінних середовища виконання в `SIMA_*`, `src/` + `include/`.
- `Retired in this pass`: 6
- `Unify` (об’єднати в меншу кількість профілів налагодження): 47
- `Framework knob` (перехід до API з чітко визначеними типами даних): 34
- `Keep as-is` (діагностика для розробників / налаштування платформи): 67

## У цьому проході завершив кар’єру.

Це були допоміжні, фіктивні або застарілі шляхи обробки, і тепер їх видалено з коду, що виконується в середовищі виконання.
...
Ці параметри налагодження значною мірою дублюють один одного, тому їх слід об’єднати в меншу кількість профілів/рівнів налагодження.
Наразі це реалізовано за допомогою `SIMA_DEBUG_PROFILE` + `SIMA_DEBUG_LEVEL` як стандартного/резервного рівня.
...
Це поведінкові параметри, які мають бути реалізовані як повноцінні типи даних в API для графів/виконання/графів/вузлів.
...
Вони все ще корисні для використання безпосередньо як змінні середовища для діагностики, початкового налаштування плагінів і низькорівневої ініціалізації.
