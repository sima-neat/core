...
| `infra.dispatcher_unavailable` | Не вдалося отримати доступ до диспетчера прискорювача, зокрема через вичерпання каналів EV74 RPMsg під час збирання. Резервний варіант на ЦП недоступний. |
...
Якщо під час запуску виникає помилка `EVXX/EV74 RPMsg capacity exhausted`, закрийте невикористані запуски графів або зупиніть конкурентні навантаження, а потім зберіть проєкт знову. Графи, які спільно використовують диспетчер, утримують його канали, доки не завершиться останній клієнт. Отримання чекає до 15 секунд за замовчуванням і налаштовується через `SIMA_RPMSG_ACQUIRE_TIMEOUT_MS`. Вилучений параметр `SIMA_RPCEVXX_IDLE_RELEASE_MS` лише попереджає і не має дії; графи в очікуванні зберігають свої резервування. Інші помилки резервування містять діагностику виявлення, дозволів або транспорту.
