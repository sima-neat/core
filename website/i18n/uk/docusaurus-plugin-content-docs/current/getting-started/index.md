...
<div class="overview-section-label">Як читати команди</div>

Кожен блок команд у цій документації позначено та кольорово закодовано відповідно до середовища, у якому він виконується, тож вам не доведеться вгадувати, де його вводити:

| Запит | Де виконувати |
| --- | --- |
| `host$` | На вашій власній машині, поза межами SDK. |
| `sdk$` | В оболонці контейнера Neat SDK. |
| `devkit$` | На Modalix DevKit. |
| `pcie-host$` | На хості з картою Modalix PCIe. |

Блок, позначений кількома середовищами, наприклад `sdk or devkit$`, виконується однаково в будь-якому з них. Якщо шляхи в команді відносні, блок також вказує каталог, з якого її слід виконувати.
