...
`yolox-seg-pose` використовує той самий шлях від MPK до статичного маніфесту, що й інші сімейства BoxDecode.
...
MLA записує свої голови у мозаїчному вигляді та, залежно від моделі, з доповненням. Хост-плагін копіює кадр із буфера драйвера. Голови, які вже є неперервними в цьому кадрі, публікуються як подання отриманого буфера; доповнені голови або голови з кроком ущільнюються в одне щільне виділення пам'яті перед публікацією. У будь-якому разі публічний тензор є щільним INT8 або BF16 у логічній формі моделі. Ущільнення спирається на наявний контракт копіювання `si_mla_read()`: маршрут коштує щонайбільше на одну хост-копію більше за стандартний і не потребує нових символів у `libsimaaipcie.so`.
