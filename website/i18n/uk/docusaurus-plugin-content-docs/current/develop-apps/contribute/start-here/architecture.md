...
Конструктори фрагментів шляху моделі SIMA встановлюють `stage-id` для елементів `simaaiprocesscvu`, `simaaiprocessmla` та `neatobjectdecode` за замовчуванням.
...
- Версія статичного маніфесту ABI передає розв’язаний контракт до `neatobjectdecode`. Плагіни.
