...
`push` очікує на вільне місце у вхідній черзі. Щоб надіслати дані без очікування, використовуйте `runner.try_push` із `Sample` або `TensorList` у C++, чи `runner.try_push_samples` / `runner.try_push_tensors` у Python. Встановіть `RunOptions.queue_depth` і `overflow_policy` до побудови runner-а. Коли обмежена черга заповнена, `KeepLatest` замінює найстарішу відкладену вибірку та повертає `true`; `Block` і `DropIncoming` повертають `false`. Закритий вхід також повертає `false`. Тримайте парні тензори одного кадру разом в одній вибірці TensorSet. Заміна не скасовує активне інференсування, а ці методи передавання не додають копіювання корисного навантаження.
