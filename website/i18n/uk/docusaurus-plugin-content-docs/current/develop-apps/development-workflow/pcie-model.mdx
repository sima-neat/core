...
Типово карта квантує вхід FP32 на EV74, виконує MLA і деквантує результати перед їх поверненням. Установіть `mla_only`, коли квантуванням керує хост: тоді карта виконує лише MLA, приймає тензори INT8 вхідного контракту MLA та повертає сирі голови INT8.

`info()` повідомляє контракт INT8 разом із параметрами квантування кожного тензора. Кожен `TensorInfo` містить `quant` з одним записом `scales` і одним записом `zero_points` на тензор, а кожен вхід також містить `input_range` — діапазон із рухомою комою, для якого модель була відкалібрована:
...
- `mla_only` вимагає архіву, скомпільованого для прямого входу та виходу INT8 MLA, надсилає кожен вхід як INT8 і повертає неперервні голови INT8.
