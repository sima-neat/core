...
Він повідомляє лише про стан життєвого циклу й не перевіряє працездатність транспорту хоста або віддаленого конвеєра.
...
Типово карта перетворює вхід FP32 на dtype MLA, виконує MLA і перетворює результати назад перед їх поверненням як FP32. `mla_only` переносить цю межу перетворення dtype до застосунку: тоді карта виконує лише MLA, хост надсилає тензори, що відповідають кожному запису `info().inputs`, і отримує власні виходи MLA в їхніх логічних формах.

`info()` описує цей контракт. Тензор INT8 містить `quant` з одним `scale` і одним `zero_point`:
...
Тензор BF16 не містить `quant`: хост округлює FP32 до BF16 з округленням до найближчого парного на вході та розширює голови BF16 на виході. У Python BF16 передається як бітові шаблони `uint16` через `Tensor.from_bytes(..., pcie.TensorDType.BFloat16, ...)` і `to_numpy()`.

Приклад нижче показує випадок INT8:
...
Маршрут працює за принципом «усе або нічого». Кожен надісланий тензор має відповідати dtype, формі та розміру в байтах відповідного запису `info().inputs`, у тому самому порядку; корисне навантаження іншого dtype відхиляється замість перетворення на карті. `mla_only` не можна поєднувати з попередньою обробкою зображень чи декодуванням рамок. Конструктор відхиляє архіви, чий етап MLA не скомпільовано для прямого входу та виходу MLA, наприклад збірку `yolo_v8s` з Model Zoo, що виконує теселяцію на CVU (`mla_only does not support stage '...' (tess)`).

Посібник [Запуск лише MLA з тензорами INT8](/tutorials/run-mla-only-int8) показує випадок INT8: він квантує зображення на хості та звіряє деквантовані голови з типовим маршрутом.
...
- `mla_only` вимагає архіву, скомпільованого для прямого входу та виходу MLA; кожен вхід надсилається в dtype свого запису `info().inputs`, а голови повертаються неперервними в dtype MLA.
