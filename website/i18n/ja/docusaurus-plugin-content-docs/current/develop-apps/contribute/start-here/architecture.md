...
- **ProcessCVU のバックエンドポリシーには唯一の権威があります。** ステージの機能判定結果が、バックエンドの利用可否と、解決処理および診断で使用される AUTO の優先設定を所有します。明示的に指定されたステージまたはセッションのターゲットは、要求されたサポート対象のバックエンドで実行されるか、失敗するかのいずれかです。後段のロールベースの上書きが、公表された AUTO の判断と暗黙的に矛盾してはなりません。
...
`yolox-seg-pose` は、他の BoxDecode ファミリーと同じ MPK から静的マニフェストへの経路を使用します。
...
MLA はヘッドをテッセレーションされた形式で書き出し、モデルによってはパディングも行います。ホストプラグインは、ドライバーバッファーからフレームをコピーします。そのフレーム内で既に連続しているヘッドは、受信バッファーへのビューとして公開されます。パディングまたはストライドを持つヘッドは、公開前に 1 つの密なアロケーションへ圧縮されます。いずれの場合も、公開されるテンソルはモデルの論理形状における密な INT8 または BF16 です。圧縮は既存の `si_mla_read()` コピー契約の上に構築されており、このルートの追加コストは既定と比べて最大でもホストコピー 1 回分であり、`libsimaaipcie.so` に新しいシンボルを必要としません。
