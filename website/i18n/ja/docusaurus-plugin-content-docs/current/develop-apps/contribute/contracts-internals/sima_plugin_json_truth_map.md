---
title: "SIMA プラグイン JSON 参照マップ（履歴）"
description: "2026年2月時点のモデルパイプライン SIMA ステージの JSON フィールド使用記録"
sidebar_position: 2
slug: /develop-apps/contribute/sima_plugin_json_truth_map
---

# SIMA プラグイン JSON 参照マップ（履歴）

_スナップショットの日付: 2026-02-17_

このドキュメントは、2026年2月時点のモデルパイプライン SIMA ステージにおける JSON フィールドの使用状況を保存したものです。以下の「現在」という表現は、その時点の状態を指します。

> **過去の契約:** 以下のプラグイン名と必須 JSON フィールドは、Platform 3.0.0 のデプロイ契約ではありません。汎用 BoxDecode は現在 `neatobjectdecode` を使用し、型付きマニフェストステージとランタイムプロパティから構成されます。以下の古い `simaaiboxdecode` の要件は、アプリケーション側の構成要件を定義するものではありません。現在の構成と互換パッケージの要件については、[SIMA 静的マニフェストコンテキスト契約](../start-here/architecture.md)を参照してください。
