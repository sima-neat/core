...
`push` は入力キューに空きができるまで待機します。待機せずに送信するには、C++ では `Sample` または `TensorList` を指定して `runner.try_push` を使用し、Python では `runner.try_push_samples` / `runner.try_push_tensors` を使用します。`RunOptions.queue_depth` と `overflow_policy` は、ランナーを構築する前に設定してください。上限のあるキューが満杯の場合、`KeepLatest` は保留中の最も古いサンプルを置き換えて `true` を返し、`Block` と `DropIncoming` は `false` を返します。入力が閉じられている場合も `false` を返します。1 フレーム分の対になるテンソルは、1 つの TensorSet サンプルにまとめてください。置き換えによって実行中の推論が取り消されることはなく、これらの転送メソッドでペイロードのコピーが追加されることもありません。
