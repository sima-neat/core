...
これはライフサイクル状態のみを報告し、ホストトランスポートやリモートパイプラインの健全性を確認するものではありません。
...
デフォルトでは、カードはFP32入力をMLAのdtypeに変換し、MLAを実行し、結果を変換し戻してからFP32として返します。`mla_only`はそのdtype変換の境界をアプリケーション側に移します。カードはMLAのみを実行し、ホストは`info().inputs`の各エントリに一致するテンソルを送信し、MLAネイティブの出力を論理形状で受け取ります。

`info()`はそのコントラクトを記述します。INT8テンソルは、1つの`scale`と1つの`zero_point`を持つ`quant`を持ちます。
...
BF16テンソルは`quant`を持ちません。ホストは入力時にFP32を最近接偶数丸めでBF16に丸め、出力時にBF16ヘッドを広げます。Pythonでは、BF16は`uint16`のビットパターンとして`Tensor.from_bytes(..., pcie.TensorDType.BFloat16, ...)`と`to_numpy()`を通ります。
...
このルートは全か無かです。送信する各テンソルは、対応する`info().inputs`エントリのdtype、形状、バイトサイズにその順序で一致する必要があります。他のdtypeのペイロードはカード上で変換されるのではなく拒否されます。`mla_only`は画像の前処理やボックスデコードと組み合わせることはできません。コンストラクターは、MLAステージが直接のMLA入出力用にコンパイルされていないアーカイブ、たとえばCVUでテッセレーションを行うModel Zooの`yolo_v8s`ビルドを拒否します（`mla_only does not support stage '...' (tess)`）。

チュートリアル[INT8テンソルでMLAのみを実行](/tutorials/run-mla-only-int8)はINT8の場合を示します。ホスト上で画像を量子化し、逆量子化したヘッドをデフォルトルートと照合します。
...
- `mla_only`には、直接のMLA入出力用にコンパイルされたアーカイブが必要です。各入力は`info().inputs`エントリのdtypeで送信され、ヘッドはMLAのdtypeで連続して返されます。
