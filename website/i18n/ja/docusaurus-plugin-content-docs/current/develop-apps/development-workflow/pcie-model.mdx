...
デフォルトでは、カードはFP32入力を量子化し、MLAを実行し、結果を逆量子化してからFP32として返します。ホストが量子化を所有する場合は`mla_only`を設定します。カードはMLAのみを実行し、MLAの入口コントラクトのINT8テンソルを受け付け、生のINT8ヘッドを返します。

BF16アクティベーションでコンパイルされたアーカイブは、量子化の代わりにキャストを行います。`mla_only`では、`quant`のないBF16テンソルを報告します。ホストは入力時にFP32を最近接偶数丸めでBF16にキャストし、出力時にBF16ヘッドを広げます。Pythonでは、BF16は`uint16`のビットパターンとして`Tensor.from_bytes(..., pcie.TensorDType.BFloat16, ...)`と`to_numpy()`を通ります。

`info()`は、すべてのテンソルの量子化パラメータとともにINT8コントラクトを報告します。各`TensorInfo`は、テンソルごとに1つの`scale`と1つの`zero_point`エントリを持つ`quant`を持ちます。
...
このルートは全か無かです。複数入力モデルは、`info().inputs`の順序で入力ごとに1つのINT8テンソルを期待します。FP32ペイロードはカード上で量子化されるのではなく拒否されます。`mla_only`は画像の前処理やボックスデコードと組み合わせることはできません。コンストラクターは、MLAステージが直接のINT8入出力用にコンパイルされていないアーカイブ、たとえばCVUでテッセレーションを行うModel Zooの`yolo_v8s`ビルドを拒否します（`mla_only does not support stage '...' (tess)`）。
...
- `mla_only`には、直接のINT8またはBF16 MLA入出力用にコンパイルされたアーカイブが必要です。すべての入力をそのdtypeで送信し、同じdtypeの連続したヘッドを返します。
