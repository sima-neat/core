...
`push`는 입력 큐에 공간이 생길 때까지 기다립니다. 기다리지 않고 제출하려면 C++에서는 `Sample` 또는 `TensorList`와 함께 `runner.try_push`를 사용하고, Python에서는 `runner.try_push_samples` / `runner.try_push_tensors`를 사용합니다. `RunOptions.queue_depth`와 `overflow_policy`는 러너를 빌드하기 전에 설정하십시오. 크기가 제한된 큐가 가득 찬 경우 `KeepLatest`는 대기 중인 가장 오래된 샘플을 교체하고 `true`를 반환하며, `Block`과 `DropIncoming`은 `false`를 반환합니다. 입력이 닫힌 경우에도 `false`를 반환합니다. 한 프레임의 짝을 이루는 텐서는 하나의 TensorSet 샘플에 함께 담으십시오. 교체는 실행 중인 추론을 취소하지 않으며, 이 전달 메서드는 페이로드 복사를 추가하지 않습니다.
