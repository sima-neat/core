...
기본적으로 카드는 FP32 입력을 양자화하고, MLA를 실행한 뒤, 결과를 역양자화하여 FP32로 반환합니다. 호스트가 양자화를 소유할 때는 `mla_only`를 설정하세요. 그러면 카드는 MLA만 실행하고, MLA 진입 계약의 INT8 텐서를 받아들이며, 원시 INT8 헤드를 반환합니다.

BF16 활성화로 컴파일된 아카이브는 양자화 대신 캐스트를 수행합니다. `mla_only`에서는 `quant`가 없는 BF16 텐서를 보고합니다. 호스트는 입력 시 FP32를 최근접 짝수 반올림으로 BF16에 캐스트하고, 출력 시 BF16 헤드를 확장합니다. Python에서 BF16은 `uint16` 비트 패턴으로 `Tensor.from_bytes(..., pcie.TensorDType.BFloat16, ...)`와 `to_numpy()`를 거칩니다.

`info()`는 모든 텐서의 양자화 매개변수와 함께 INT8 계약을 보고합니다. 각 `TensorInfo`는 텐서마다 하나의 `scale`와 하나의 `zero_point` 항목을 가진 `quant`를 가집니다.
...
이 경로는 전부 아니면 전무입니다. 다중 입력 모델은 `info().inputs`의 순서로 입력마다 하나의 INT8 텐서를 기대합니다. FP32 페이로드는 카드에서 양자화되는 대신 거부됩니다. `mla_only`는 이미지 전처리나 박스 디코드와 결합할 수 없습니다. 생성자는 MLA 단계가 직접 INT8 입출력용으로 컴파일되지 않은 아카이브, 예를 들어 CVU에서 테셀레이션을 수행하는 Model Zoo의 `yolo_v8s` 빌드를 거부합니다(`mla_only does not support stage '...' (tess)`).
...
- `mla_only`에는 직접 INT8 또는 BF16 MLA 입출력용으로 컴파일된 아카이브가 필요하며, 모든 입력을 해당 dtype으로 제출하고 같은 dtype의 연속된 헤드를 반환합니다.
