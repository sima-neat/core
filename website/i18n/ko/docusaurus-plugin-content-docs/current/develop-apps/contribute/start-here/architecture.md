...
Graph 및 Model 빌드는 페이로드 프리플라이트가 비활성화된 경우에도 시작이 반환되기 전, 디스패처 초기화 중에 EV74 RPMsg 채널을 예약합니다. 각 워커는 고유한 엔드포인트를 소유합니다. 동일한 프로세스의 Graph는 디스패처를 공유하며, 마지막 클라이언트가 종료되면 해당 채널이 해제됩니다. 유휴 상태가 지속되어도 해제되지 않습니다.

용량이 고갈되면 빌드는 `infra.dispatcher_unavailable`로 실패합니다. 오류 상태의 디스패처는 새 Graph를 수락할 수 없으므로, 다시 빌드하기 전에 모든 클라이언트를 종료하십시오. 구현과 채널 잠금은 Core가 아니라 Neat Internals에 속합니다.
...
- **ProcessCVU 백엔드 정책에는 단일 권한이 있습니다.** 스테이지의 기능 판정 결과가 백엔드 가용성과 해석 및 진단에 사용되는 AUTO 기본 설정을 소유합니다. 명시적으로 지정된 스테이지 또는 세션 대상은 요청된 지원 백엔드에서 실행되거나 실패합니다. 이후의 역할 기반 재정의가 공표된 AUTO 결정과 조용히 모순되어서는 안 됩니다.
...
`VideoRate()`는 소스 그룹에 의해 삽입된 경우를 포함해 항상 `videorate drop-only=true`를 생성합니다. 이를 통해 풀링된 디코더 메모리를 공유하는 출력 헤더의 중복을 방지하고 소스 타임스탬프를 유지합니다. C++의 `VideoRate()`와 Python의 `video_rate()` 팩토리는 인수를 받지 않으며 프레임을 복제하지 않습니다.
...
YOLOv5 검출은 표준 앵커를 사용하는 고정 프로파일입니다. Core는 stride 8/16/32 순서로 정렬된 원시 패킹 텐서가 정확히 3개 필요하며, 각각의 논리적 깊이는 `3 * (num_classes + 5)`여야 합니다. 점수 영역을 시그모이드로 정규화하고, 모순되는 클래스 수는 하위 변환 전에 거부합니다. 런타임은 모델과 헤드 형상에서 직사각형 x/y stride를 도출하고 표준 YOLOv5 앵커 테이블을 적용합니다. 사용자 지정 AutoAnchor 테이블은 이 계약의 범위를 벗어납니다.
...
`yolox-seg-pose`는 다른 BoxDecode 계열과 동일한 MPK-정적 매니페스트 경로를 사용합니다.
...
RTP JPEG의 경우 `rtpjpegdepay` 뒤의 호환성 프로브가 구문 분석 전에 누락된 JPEG 종료 마커를 추가합니다. 올바르게 종료된 이미지는 변경 없이 통과하며, 이 기능은 패킷 손실이나 기타 잘못된 JPEG 데이터를 복구하지 않습니다.
...
`ModelOptions::mla_only`는 실행 경계를 이동합니다. 카드는 MLA 스테이지만 실행하며, 그 앞의 양자화도 뒤의 역양자화도 수행하지 않으므로 두 변환은 애플리케이션이 담당합니다. `Model::info()`는 이에 필요한 정보를 게시합니다. INT8 아카이브에서는 모든 `TensorInfo`가 스케일 하나와 영점 하나를 가진 `quant`를 포함하며, BF16 아카이브는 양쪽 경계에서 캐스팅하고 아무것도 게시하지 않습니다. 이 경로는 INT8 또는 BF16 입력만 허용하므로 FP32 텐서는 조용히 변환되지 않고 거부됩니다.

이 옵션은 모델 옵션 JSON의 `execution.mla_only`로 카드에 전달됩니다. `InputKind::Tensor`가 필요하며, 이 경로가 실행하지 않는 스테이지를 구성하는 전처리 및 박스 디코딩 옵션은 거부됩니다.

MLA는 헤드를 테셀레이션된 형태로 기록하며, 모델에 따라 패딩이 추가됩니다. 호스트 플러그인은 드라이버 버퍼에서 프레임을 복사합니다. 해당 프레임 내에서 이미 연속된 헤드는 수신 버퍼에 대한 뷰로 게시되고, 패딩되었거나 스트라이드가 있는 헤드는 게시 전에 하나의 조밀한 할당으로 압축됩니다. 어느 경우든 공개되는 텐서는 모델의 논리적 형상을 갖는 조밀한 INT8 또는 BF16입니다. 압축은 기존 `si_mla_read()` 복사 계약 위에서 이루어지며, 이 경로는 기본값보다 최대 호스트 복사 한 번의 비용만 추가되고 `libsimaaipcie.so`에 새로운 심볼을 요구하지 않습니다.
