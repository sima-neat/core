...
Graph 與 Model 的建置會在訊息分派器初始化期間、啟動程序返回之前保留 EV74 RPMsg 通道，即使已停用酬載預檢也是如此。每個工作處理序各自擁有獨立的端點。同一個處理序中的 Graph 會共用訊息分派器；最後一個客戶端關閉時才會釋放其通道。閒置期間不會釋放。

容量耗盡會使建置以 `infra.dispatcher_unavailable` 失敗。發生錯誤的訊息分派器無法接受新的 Graph；請在重新建置之前關閉其所有客戶端。實作與通道鎖屬於 Neat Internals，而非 Core。
...
MLA 會以鑲嵌方式寫出其標頭，並視模型而定加上填充。主機外掛程式會將影格從驅動程式緩衝區複製出來。在該影格中已經連續的標頭會以接收緩衝區的檢視形式發布；有填充或跨距的標頭則會在發布前壓縮為單一的密集配置。無論何者，公開的張量都是模型邏輯形狀下的密集 INT8 或 BF16。壓縮建立在既有的 `si_mla_read()` 複製合約之上：此路由最多只比預設多一次主機複製，且不需要 `libsimaaipcie.so` 的新符號。
