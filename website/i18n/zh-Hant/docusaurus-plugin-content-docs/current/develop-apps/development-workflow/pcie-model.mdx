...
預設情況下，卡會量化 FP32 輸入、執行 MLA，並在以 FP32 傳回結果前將其反量化。當主機自行掌握量化時，請設定 `mla_only`：此時卡只執行 MLA，接受符合 MLA 進入合約的 INT8 張量，並傳回原始的 INT8 輸出頭。

以 BF16 活化值編譯的封存檔會進行型別轉換而非量化。在 `mla_only` 下，它會回報沒有 `quant` 的 BF16 張量：主機在輸入時以四捨五入至最接近偶數的方式將 FP32 轉換為 BF16，並在輸出時將 BF16 輸出頭擴展回來。在 Python 中，BF16 以 `uint16` 位元模式透過 `Tensor.from_bytes(..., pcie.TensorDType.BFloat16, ...)` 與 `to_numpy()` 傳遞。

`info()` 會連同每個張量的量化參數一起回報 INT8 合約。每個 `TensorInfo` 都帶有 `quant`，其中每個張量各有一個 `scale` 與一個 `zero_point` 項目：
...
此路徑是全有或全無。多輸入模型期望依 `info().inputs` 的順序為每個輸入提供一個 INT8 張量；FP32 有效負載會被拒絕，而不會在卡上被量化。`mla_only` 無法與影像前處理或方框解碼併用。建構函式會拒絕 MLA 階段未針對直接 INT8 輸入輸出編譯的封存檔，例如在 CVU 上進行鑲嵌的 Model Zoo `yolo_v8s` 建置（`mla_only does not support stage '...' (tess)`）。
...
- `mla_only` 需要為直接 INT8 或 BF16 MLA 輸入輸出而編譯的封存檔，會以該 dtype 提交每個輸入，並傳回相同 dtype 的連續輸出頭。
