...
if [[ -f .github/workflows/release-gate.yml ]] &&
   ! grep -q "docs/develop-apps/contribute/release-checklist.md" .github/workflows/release-gate.yml; then
  echo "ERROR: release-gate workflow must reference docs/develop-apps/contribute/release-checklist.md" >&2
  fail=1
fi
...
for check_name in "${required_checks[@]}"; do
  if ! grep -q "${check_name}" docs/develop-apps/contribute/release-checklist.md; then
    echo "ERROR: docs/develop-apps/contribute/release-checklist.md must list required check: ${check_name}" >&2
    fail=1
  fi
done
