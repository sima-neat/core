...
This chapter configures a YOLOv8 model end to end, then inspects the contract the runtime resolved from those options. By the end you will have set input, preprocessing, and postprocessing knobs, read back the resolved `input_spec`/`output_spec`/`metadata`, and run one deterministic frame through the configured model.
