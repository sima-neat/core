...
int main(int argc, char** argv) {
  try {
    ...
    const fs::path root = find_repo_root();
    std::string mpk_arg;
    fs::path mpk_path = get_arg(argc, argv, "--mpk", mpk_arg) ? fs::path(mpk_arg)
                                                              : first_existing({
                                                                    default_yolo_mpk(),
                                                                    default_resnet_mpk(),
                                                                });
    ...
    } else if (wants_print_gst(argc, argv)) {
    ...
    // END CORE LOGIC
    require(out.tensor.has_value(), "direct session output missing tensor");
