...
This chapter uses the async push/pull surface. To measure the same model with deterministic synthetic inputs, continue to [Benchmark Your Model](/tutorials/benchmark-your-model). For the full build-vs-run and sync-vs-async model plus the complete `RunOptions` surface, see [Build Your First Graph](/tutorials/build-inference-pipeline). For queue depth, overflow policy, and measurement under load, see [Tune Throughput and Queue Depth](/tutorials/tune-throughput-and-queues).
