 ...
 * @file
 * @ingroup nodes_sima
 * @brief `DetessCast` Node — fused CVU kernel that detessellates and casts BF16→FP32.
 *
 * Combines an MLA-layout untessellate with a BF16→FP32 cast in a single CVU pass. Inserted
 * after MLA stages on the BF16 output path when downstream wants natural HWC/CHW float
 * tensors — fusing the two stages saves a DDR round-trip versus running `Detess` then `Cast`.
 *
...
/**
 * @brief Construction options for a `DetessCast` Node.
 *
 * @ingroup nodes_sima
 */
struct DetessCastOptions {
  ...
  explicit DetessCastOptions(const simaai::neat::Model& model);
...
/**
 * @brief Fused CVU kernel Node: `Detess` followed by `Cast` (BF16→FP32).
 *
 * The route planner picks `DetessCast` after MLA stages whenever the BF16 path needs
 ...
 * @ingroup nodes_sima
 */
class DetessCast final : public Node,
...
public:
  ...
  explicit DetessCast(DetessCastOptions opt = {});
  struct ConfigHolder;
