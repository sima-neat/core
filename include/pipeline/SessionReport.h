 ...
 *
 * @see SessionError for the exception type that carries this report
 ...
 */
struct NodeReport {
  int index = -1;                    ///< Position in the Session's Node list.
 ...
 *
 * When a Session is built with a sample input, the framework adapts the pipeline's caps to
 ...
 *
 * Returned by `Session::validate()` (always) and carried by `SessionError::report()` (when
 ...
 * the framework.
 *
 * @see SessionError, ValidateOptions, "Error code taxonomy" (§41)
 * @ingroup diagnostics
 */
