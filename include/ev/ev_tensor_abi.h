...
static inline enum sima_ev_run_target sima_ev_requested_run_target(
    const struct sima_ev_abi_header* hdr)
{
    if (!hdr) {
...
static inline int64_t sima_ev_axis_stride_bytes(
    const struct sima_ev_shape_desc* shape,
...
static inline int64_t sima_ev_tensor_axis_stride_bytes(
    const struct sima_ev_tensor_desc* desc,
    enum sima_ev_axis_semantic axis,
    int64_t fallback)
{
...
static inline int64_t sima_ev_tensor_tile_axis_size(
    const struct sima_ev_tensor_desc* desc,
    enum sima_ev_axis_semantic axis,
    int64_t fallback)
{
...
static inline enum sima_ev_dense_tensor_format sima_ev_infer_dense_tensor_format(
    const struct sima_ev_tensor_desc* desc)
{
    if (!desc || desc->layout_kind != SIMA_EV_LAYOUT_STRIDED) {
    ...
    const int64_t stride_d =
        sima_ev_tensor_axis_stride_bytes(desc, SIMA_EV_AXIS_D,
                                         (height > 0 && width > 0 && channels > 0)
                                             ? height * width * channels * elem_size
                                             : -1);
    const int64_t stride_h =
        sima_ev_tensor_axis_stride_bytes(desc, SIMA_EV_AXIS_H,
                                         (width > 0 && channels > 0)
                                             ? width * channels * elem_size
                                             : -1);
    const int64_t stride_w =
        sima_ev_tensor_axis_stride_bytes(desc, SIMA_EV_AXIS_W,
                                         (channels > 0) ? channels * elem_size : -1);
    ...
    if (stride_c == ndhwc_stride_c && stride_w == ndhwc_stride_w &&
        stride_h == ndhwc_stride_h && stride_d == ndhwc_stride_d) {
    ...
    if (stride_w == ndchw_stride_w && stride_h == ndchw_stride_h &&
        stride_c == ndchw_stride_c && stride_d == ndchw_stride_d) {
...
static inline int sima_ev_tensor_is_contiguous(const struct sima_ev_tensor_desc* desc)
{
    ...
    for (uint32_t idx = desc->shape.rank; idx-- > 0U;) {
        ...
        if (desc->shape.sizes[idx] > 0 &&
            expected_stride <= (INT64_MAX / desc->shape.sizes[idx])) {
