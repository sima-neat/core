...
bool can_encode_nv12_direct(const OutputSpec& input, bool simaai_layout_aware) {
   ...
   * SystemMemory uses the explicit materializing variant of the same node;
   * Core copies once into standard CMA with the required raster layout.
   * There is no implicit encoder upload or redundant color conversion. */
