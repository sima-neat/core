...
Model::ModelInfo Model::info() const {
  ...
  out.selection.infer_only =
      !out.selection.include_preprocess_stage && !out.selection.include_postprocess_stage;
...
Graph ModelAccess::build_graph_fragment(const Model& model, Model::RouteOptions opt,
  ...
  add_nodes_to_graph(graph, build_pipeline_nodes(model, pack, model.impl_->options, render_plan,
                                                 opt, nullptr, false, false));
