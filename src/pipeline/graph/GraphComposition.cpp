...
void ignore_inflight_options_for_default_policy(GraphLinkOptions& options) {
  if (options.policy != GraphLinkPolicy::Default) {
    return;
  }
  options.max_inflight_per_stream = -1;
  options.max_inflight_total = -1;
}

void merge_realtime_link_options(GraphLinkOptions& existing, GraphLinkOptions& incoming) {
  ignore_inflight_options_for_default_policy(existing);
  ignore_inflight_options_for_default_policy(incoming);
