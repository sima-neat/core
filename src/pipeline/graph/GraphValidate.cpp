 ...
 * @brief Graph validation helpers.
 *
...
GraphReport Graph::validate(const ValidateOptions& opt) const {
  gst_init_once();
  ...
  if (composition_ && !composition_->is_linear()) {
    ...
    try {
      ...
      rep.pipeline_string = "<connected graph validation failed>";
      rep.error_code = error_codes::kPipelineShape;
      rep.repro_note = format_validate_note(
          "Graph::validate", rep.error_code, "connected graph validation failed", e.what(),
          "Check endpoint names, Branch/Combine policy, and public graph connectivity.");
      return rep;
    }
  }

  const auto nodes = linear_nodes_snapshot("Graph::validate");
  enforce_caps_behavior(nodes, "Graph::validate");

  GraphReport rep;

  try {
    throw_if_input_appsrc_present(nodes, "Graph::validate");
  } catch (const NeatError& e) {
...

  {
    ...
    ValidationReport vrep = reg.validate(validate_nodes, ctx);
    if (vrep.has_errors()) {
                                       ...
                                       name_transform, &opt_);
  {
    ...
    compile_input.pipeline_label = "Graph::validate";
  ...
  if (!sink) {
    ...
    rep.error_code = error_codes::kPipelineShape;
    rep.repro_note = format_validate_note("Graph::validate", rep.error_code, "appsink not found",
                                          "element='" + appsink_name + "' pipeline_snippet='" +
                                              pipeline_snippet(br.pipeline_string) + "'",
                                          "Ensure Output() is the final node in this Graph.");
    stop_and_unref(pipeline);
    return rep;
  }

  try {
    set_state_or_throw(pipeline, GST_STATE_PAUSED, "Graph::validate", br.diag);
  } catch (const NeatError& e) {
    rep = e.report();
    ...
    rep.repro_note = format_validate_note(
        "Graph::validate", rep.error_code, "failed to PAUSE/preroll", rep.repro_note,
...
GraphReport Graph::validate(const ValidateOptions& opt, const cv::Mat& input) const {
  gst_init_once();

  const auto nodes = linear_nodes_snapshot("Graph::validate(input)");
  enforce_caps_behavior(nodes, "Graph::validate(input)");

  GraphReport rep;

  const Input* src_node = nullptr;
  try {
    require_input_appsrc(nodes, "Graph::validate(input)", &src_node);
  } catch (const NeatError& e) {
    rep.pipeline_string = "<input required>";
    rep.error_code = e.report().error_code;
    rep.repro_note = e.what();
  ...
  {
    const std::vector<std::shared_ptr<Node>> validate_nodes =
        session_build_materialize_model_bound_nodes(nodes, true);
    ValidationContext ctx;
    ctx.mode = ValidationContext::Mode::Validate;
    ctx.strict = true;
  ...
  {
    ContractCompileInput compile_input;
    compile_input.pipeline_label = "Graph::validate(input)";
    compile_input.processcvu_requested_run_target = opt_.processcvu_requested_run_target;
    compile_input.processcvu = opt_.processcvu;
  ...
  if (!sink) {
    ...
    rep.error_code = error_codes::kPipelineShape;
    rep.repro_note =
        format_validate_note("Graph::validate(input)", rep.error_code, "appsink not found",
                             "element='" + appsink_name + "' pipeline_snippet='" +
                                 pipeline_snippet(br.pipeline_string) + "'",
                             "Ensure Output() is the final node in this Graph.");
    stop_and_unref(pipeline);
  ...
  if (!appsrc) {
    rep.error_code = error_codes::kPipelineShape;
    rep.repro_note =
        format_validate_note("Graph::validate(input)", rep.error_code, "appsrc not found",
                             "element='" + appsrc_name + "' pipeline_snippet='" +
                                 pipeline_snippet(br.pipeline_string) + "'",
                             "Ensure Input() is the first node in this Graph.");
    gst_object_unref(sink);
    stop_and_unref(pipeline);
    return rep;
  }

  const SampleSpec spec = infer_input_spec(src_node->options(), input, "Graph::validate(input)");
  ...
  try {
    set_state_or_throw(pipeline, GST_STATE_PAUSED, "Graph::validate(input)", br.diag);
  } catch (const NeatError& e) {
    rep = e.report();
    if (rep.error_code.empty()) {
      rep.error_code = error_codes::kCaps;
    ...
    rep.repro_note = format_validate_note(
        "Graph::validate(input)", rep.error_code, "failed to PAUSE/preroll", rep.repro_note,
        "Inspect bus diagnostics and offending caps negotiation near the first GST ERROR.");
  ...
  if (!buf) {
    ...
    rep.repro_note = format_validate_note(
        "Graph::validate(input)", rep.error_code, "failed to allocate GstBuffer",
  ...
  if (!gst_buffer_map(buf, &mi, GST_MAP_WRITE)) {
    ...
    rep.error_code = error_codes::kInputShape;
    rep.repro_note =
        format_validate_note("Graph::validate(input)", rep.error_code, "failed to map GstBuffer",
                             "required_bytes=" + std::to_string(spec.required_bytes_actual),
  ...
  if (spec.required_bytes_actual > contiguous_bytes) {
    ...
    rep.repro_note = format_validate_note(
        "Graph::validate(input)", rep.error_code, "input frame smaller than required bytes",
  ...
  if (spec.kind == SampleMediaKind::RawVideo) {
    ...
    if (!pipeline_internal::attach_video_meta(&meta_buf, spec, &meta_err)) {
      ...
      rep.error_code = error_codes::kCaps;
      rep.repro_note = format_validate_note(
          "Graph::validate(input)", rep.error_code, "failed to attach video meta",
  ...
  if (push_ret != GST_FLOW_OK) {
    ...
    std::string hint = "Inspect GraphReport bus diagnostics for the first terminal GST error.";
    ...
    rep.error_code = error_codes::kCaps;
    rep.repro_note = format_validate_note("Graph::validate(input)", rep.error_code,
