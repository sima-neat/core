  ...
  for (char c : in) {
    ...
    default:
      if (static_cast<unsigned char>(c) < 0x20) {
        char esc[8];
        std::snprintf(esc, sizeof(esc), "\\u%04x", static_cast<unsigned int>(c) & 0xFFu);
        out += esc;
      } else {
        out += c;
      }
...
std::string LatencyProfiler::to_text(const ProfilerReport& r) {
  ...
  o << "frames:     " << r.frames_total << "   warmup:  " << r.warmup_frames << "\n";
  o << "ring_emits: " << r.profiler_emits << "   dropped: " << r.profiler_dropped << "\n";
  o << "\n";

  o << "[end-to-end]\n"
    << "  pushed=" << r.end_to_end.inputs_pushed << " pulled=" << r.end_to_end.outputs_pulled
  ...
  o << "[input stream]\n"
    << "  push_count=" << r.input_stream.push_count << " pull_count=" << r.input_stream.pull_count
  ...
  if (!r.kernel_aggregates.empty()) {
    ...
    for (const auto& a : r.kernel_aggregates) {
      o << "  " << std::left << std::setw(8) << a.backend << " " << std::right << std::setw(7)
        ...
        << fmt_ms(a.total_ms) << "  " << a.kernel_name;
  ...
  if (!r.diag.element_timings.empty()) {
    ...
    for (const auto& e : r.diag.element_timings) {
      const double avg_us =
          e.samples > 0 ? (static_cast<double>(e.total_us) / static_cast<double>(e.samples)) : 0.0;
      o << "  " << std::left << std::setw(28) << e.element_name << " samples=" << e.samples
        << " avg=" << fmt_ms(avg_us / 1000.0)
        << " max=" << fmt_ms(static_cast<double>(e.max_us) / 1000.0) << " ms\n";
    }
    o << "\n";
  }
  ...
  if (!r.diag.element_pad_timings.empty()) {
    ...
    std::sort(sorted.begin(), sorted.end(),
              [](const RunElementPadTimingStats& a, const RunElementPadTimingStats& b) {
                if (a.element_name != b.element_name)
                  return a.element_name < b.element_name;
                if (a.is_sink != b.is_sink)
                  return a.is_sink && !b.is_sink;
    ...
    for (const auto& p : sorted) {
      ...
      const double avg_inter_us =
          p.samples > 1
              ? (static_cast<double>(p.inter_arrival_total_us) / static_cast<double>(p.samples - 1))
              : 0.0;
      const double avg_qw_us = p.queue_wait_samples > 0
                                   ? (static_cast<double>(p.queue_wait_total_us) /
                                      static_cast<double>(p.queue_wait_samples))
                                   : 0.0;
      o << "  " << std::left << std::setw(30) << p.element_name << " " << std::setw(10)
        << p.pad_name << " " << std::setw(5) << (p.is_sink ? "sink" : "src") << " " << std::right
        << std::setw(7) << p.samples << "  " << std::setw(12) << fmt_ms(avg_inter_us / 1000.0)
        << "  " << std::setw(12) << fmt_ms(static_cast<double>(p.inter_arrival_max_us) / 1000.0)
        << "  " << std::setw(17) << fmt_ms(avg_qw_us / 1000.0) << "  " << std::setw(17)
        << fmt_ms(static_cast<double>(p.queue_wait_max_us) / 1000.0) << "  " << fmt_bytes(p.bytes)
        << "\n";
    }
    o << "\n";
  }
  ...
  if (any_memcpy) {
    ...
    for (const auto& m : r.memcpy_sites) {
      if (m.calls == 0 && m.total_ns == 0)
        continue;
      o << "  " << std::left << std::setw(26) << m.site_name << " " << std::right << std::setw(5)
        << m.calls << "   " << std::setw(8) << fmt_ms(m.total_ms()) << "   " << std::setw(6)
        << fmt_ms(static_cast<double>(m.max_ns) / 1.0e6) << "  " << fmt_bytes(m.total_bytes)
        << "\n";
    }
  }
