 ...
 * @brief RTSP server support for Session.
 *
...
RtspServerHandle Session::run_rtsp(const RtspServerOptions& opt) {
  gst_init_once();

  enforce_caps_behavior(nodes_, "Session::run_rtsp");

  require_element("appsrc", "Session::run_rtsp");
  require_element("rtph264pay", "Session::run_rtsp");
  require_element("h264parse", "Session::run_rtsp");
  ...
  if (!src_img) {
    throw std::runtime_error("Session::run_rtsp: missing StillImageInput node");
  }
  ...
  for (size_t i = 0; i < nodes_.size(); ++i) {
    if (i)
      ss << " ! ";
    NodeFragment frag = make_node_fragment(nodes_[i], static_cast<int>(i), name_transform);
    ss << frag.fragment;
  }
