...
Run::Run(std::shared_ptr<State> state) : state_(std::move(state)) {
  if (state_) {
    state_->handle_refs.fetch_add(1, std::memory_order_relaxed);
    owns_ref_ = true;
  }
}

Run::Run(Run&& other) noexcept : state_(std::move(other.state_)), owns_ref_(other.owns_ref_) {
...
Run Run::create(InputStream stream, const RunOptions& opt, const InputStreamOptions& stream_opt,
  ...
  st->zero_copy_fallback_enabled = (opt.preset == RunPreset::Balanced) && !stream_opt.copy_output;
  ...
  if (pipeline_internal::env_bool("SIMA_PIPELINE_DEBUG", false) ||
    ...
    std::fprintf(stderr, "[PIPELINE] create supports_push=%s supports_pull=%s\n",
                 st->supports_push ? "true" : "false", st->supports_pull ? "true" : "false");
  }
  if (st->diag_enabled) {
  ...
  auto on_output = [st](Sample out) {
...

    {
      ...
      if (!st->copy_output_latched.load(std::memory_order_relaxed) &&
          st->zero_copy_fallback_enabled && has_zero_copy_output && max > 0 &&
          static_cast<int>(st->out_queue.size()) >= max) {
        st->copy_output_latched.store(true, std::memory_order_relaxed);
        if (!st->zero_copy_warned.exchange(true, std::memory_order_relaxed)) {
          std::fprintf(stderr, "[WARN] Balanced preset: zero-copy output reliability trip; "
                               "switching to copy-output mode.\n");
        }
      }
      const bool copy_output = st->copy_output_latched.load(std::memory_order_relaxed);
      ...
      if (max > 0) {
        ...
        if (output_drop == OverflowPolicy::Block) {
          st->out_cv.wait(lock, [&]() {
            return st->stop_requested.load() || static_cast<int>(st->out_queue.size()) < max;
          });
        } else if (queue_full(st->out_queue, max)) {
  ...
  st->input_thread = std::thread([st]() {
    ...
    while (true) {
      ...
      std::size_t q_after_pop = 0;
      {
        ...
        st->in_cv.wait(lock, [&]() {
          return st->stop_requested.load() || st->input_closed || !st->in_queue.empty();
        });
        if (st->stop_requested.load())
          break;
