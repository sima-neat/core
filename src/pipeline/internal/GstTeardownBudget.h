...
inline int synchronous_live_teardown_budget_ms(int base_timeout_ms,
  ...
  const std::uint64_t bounded_base_ms =
      static_cast<std::uint64_t>(std::clamp(base_timeout_ms, 0, kMaximumBudgetMs));
  ...
  const std::uint64_t requested_ms = bounded_base_ms + rtsp_timeout_ms + scheduling_margin_ms;
