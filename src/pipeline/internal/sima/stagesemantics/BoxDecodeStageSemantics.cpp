...
build_boxdecode_compiled_contract(const BoxDecodeStaticContract& contract) {
  ...
  if (contract.decode_type_option != BoxDecodeTypeOption::Auto) {
    options.decode_type_option = contract.decode_type_option;
  }
  options.score_activation = contract.score_activation;
  options.detection_threshold = contract.detection_threshold;
  options.nms_iou_threshold = contract.nms_iou_threshold;
  options.topk = contract.topk;
  options.num_classes = contract.num_classes;
  options.model_owned_flags = contract.model_owned_flags;
  options.quant_contract_required = contract.quant_contract_required;
  options.required_preprocess_meta_fields = contract.required_preprocess_meta_fields;
