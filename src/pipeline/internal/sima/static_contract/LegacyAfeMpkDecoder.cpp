...
Node parse_node(const Json& value, const std::string& path) {
  if (!value.is_object()) {
    reject(LegacyAfeDecodeErrorCode::InvalidField, path, "expected a node object");
  }
  Node result;
  result.name = required_string(value, "name", path);
  result.bytes = positive_u64(*required_member_ptr(value, "size", path), path + ".size");
  if (value.contains("dtype")) {
    if (!value.at("dtype").is_string() || value.at("dtype").get_ref<const std::string&>().empty()) {
      reject(LegacyAfeDecodeErrorCode::InvalidField, path + ".dtype",
             "expected a non-empty string");
    }
    result.dtype = value.at("dtype").get<std::string>();
  }
  if (value.contains("shape")) {
    result.shape = shape(value.at("shape"), path + ".shape");
  }
  if (value.contains("layout")) {
    if (!value.at("layout").is_string() ||
        value.at("layout").get_ref<const std::string&>().empty()) {
      reject(LegacyAfeDecodeErrorCode::InvalidField, path + ".layout",
             "expected a non-empty string");
    }
    result.layout = value.at("layout").get<std::string>();
  }
  return result;
}
...
std::vector<Node> nodes(const Json& object, const char* key, const std::string& path) {
  ...
  result.reserve(value.size());
  for (std::size_t index = 0; index < value.size(); ++index) {
    const auto& node_json = value[index];
    const std::string node_path = path + "." + key + "[" + std::to_string(index) + "]";
    result.push_back(parse_node(node_json, node_path));
  }
  return result;
}
...
std::optional<std::vector<std::int64_t>> qmla_dense_last_axis_strides(
  ...
  if (!value.logical_dtype.has_value() || !value.logical_shape.has_value() ||
      value.logical_shape->empty() || value.logical_layout != std::optional<std::string>{"normal"}) {
    return std::nullopt;
  }
  ...
  if (!width.has_value() || !logical_bytes.has_value() ||
      *logical_bytes != value.required_bytes) {
    return std::nullopt;
  }

  const auto last_dimension = value.logical_shape->back();
  if (last_dimension <= 0 ||
      static_cast<std::uint64_t>(last_dimension) >
          std::numeric_limits<std::uint64_t>::max() / *width) {
    return std::nullopt;
  }
  const auto padded_last_axis =
      align_up_16(static_cast<std::uint64_t>(last_dimension) * *width);
  if (!padded_last_axis.has_value()) {
    return std::nullopt;
  }
  std::uint64_t prefix_elements = 1U;
  for (std::size_t axis = 0; axis + 1U < value.logical_shape->size(); ++axis) {
    const auto dimension = value.logical_shape->at(axis);
    if (dimension <= 0 || prefix_elements >
                              std::numeric_limits<std::uint64_t>::max() /
                                  static_cast<std::uint64_t>(dimension)) {
      return std::nullopt;
    }
    prefix_elements *= static_cast<std::uint64_t>(dimension);
  }
  if (prefix_elements > std::numeric_limits<std::uint64_t>::max() / *padded_last_axis ||
      prefix_elements * *padded_last_axis != physical_extent) {
    return std::nullopt;
  }
  ...
  for (std::size_t reverse = value.logical_shape->size(); reverse > 0U; --reverse) {
    ...
    if (axis + 1U == value.logical_shape->size()) {
      ...
      stride = *padded_last_axis;
    } else {
      const auto inner_dimension = value.logical_shape->at(axis + 1U);
      if (inner_dimension <= 0 || stride >
                                      std::numeric_limits<std::uint64_t>::max() /
                                          static_cast<std::uint64_t>(inner_dimension)) {
        return std::nullopt;
      }
      stride *= static_cast<std::uint64_t>(inner_dimension);
    }
...
void merge_node_evidence(ValueSpec& value, const Node& node, const std::string& path) {
  if (node.dtype.has_value()) {
    merge_dtype(value, *node.dtype, path + ".dtype");
  }
  if (node.shape.has_value()) {
    merge_shape(value, *node.shape, path + ".shape");
  }
  if (node.layout.has_value()) {
    if (value.logical_layout.has_value() && value.logical_layout != node.layout) {
      reject(LegacyAfeDecodeErrorCode::ConfigurationMismatch, path + ".layout",
             "conflicting exact layout evidence for value '" + value.name + "'");
    }
    value.logical_layout = node.layout;
  }
}
...
bool is_direct_terminal_publication(const ModelExecutionPlanData& data,
                                    const std::size_t mla_op_index,
                                    const std::size_t publication_op_index,
                                    const ValueId value_id) {
  if (publication_op_index >= data.ops.size() || mla_op_index >= publication_op_index) {
    return false;
  }
  const auto& publication = data.ops[publication_op_index];
  if (std::find(publication.inputs.begin(), publication.inputs.end(), value_id) ==
      publication.inputs.end()) {
    return false;
  }
  for (std::size_t index = mla_op_index + 1U; index < publication_op_index; ++index) {
    if (std::find(data.ops[index].inputs.begin(), data.ops[index].inputs.end(), value_id) !=
        data.ops[index].inputs.end()) {
      return false;
    }
  }
  return true;
...
void author_mla_output_storage_binding(ModelExecutionPlanData& data,
                                       const std::size_t mla_op_index,
                                       const std::size_t publication_op_index,
                                       ...
                                       std::vector<LegacyAfeProofFact>* proof) {
  const auto& mla = data.ops.at(mla_op_index);
  auto& value = data.values.at(mla.outputs.at(output_index));
  const std::string path = "$.plugins[" + std::to_string(mla_op_index) + "].output_nodes[" +
                           std::to_string(output_index) + "]";
  if (physical_extent < value.required_bytes) {
    reject(LegacyAfeDecodeErrorCode::ValueSizeMismatch, path + ".size",
           "QMLA OFM physical extent is smaller than the MPK logical tensor");
  }
  if (physical_extent == value.required_bytes) {
    return;
  }
  ...
  if (const auto strides = qmla_dense_last_axis_strides(value, physical_extent)) {
    value.read_expression = ReadExpression{value.id, 0U, *strides};
    if (proof) {
      proof->push_back(
          {"MLA.OFM[" + std::to_string(output_index) + "].layout",
           "QMLA SHT_DATA extent " + std::to_string(physical_extent) +
               " exactly proves 16-byte dense-last-axis row pitch for logical value '" +
               value.name + "'"});
    }
    return;
  }

  if (!is_direct_terminal_publication(data, mla_op_index, publication_op_index, value.id) ||
      !value.logical_shape.has_value() || !value.logical_dtype.has_value() ||
      value.logical_layout != std::optional<std::string>{"normal"}) {
    reject(LegacyAfeDecodeErrorCode::ConfigurationMismatch, path,
           "larger MLA output carrier is neither exact row padding nor provable terminal tail "
           "padding");
  }
  const auto width = element_width(*value.logical_dtype);
  const auto logical_bytes = dense_bytes(*value.logical_shape, *value.logical_dtype);
  const auto strides = width.has_value()
                           ? contiguous_stride_bytes(*value.logical_shape, *width)
                           : std::nullopt;
  if (!logical_bytes.has_value() || *logical_bytes != value.required_bytes ||
      !strides.has_value()) {
    reject(LegacyAfeDecodeErrorCode::ConfigurationMismatch, path,
           "terminal MLA output has no exact dense logical view over its larger carrier");
  }
  value.read_expression = ReadExpression{value.id, 0U, *strides};
  if (proof) {
    proof->push_back(
        {"MLA.OFM[" + std::to_string(output_index) + "].layout",
         "QMLA SHT_DATA extent " + std::to_string(physical_extent) +
             " exceeds the exact dense terminal view; surplus bytes are compiler-authored tail "
             "padding"});
  }
}
...
LegacyAfeDecodeResult decode_impl(const std::string_view text, const MlaElfIoTopology& topology,
  ...
  try {
    ...
    for (std::size_t index = 0; index < input_array.size(); ++index) {
      ...
      const auto input_node = parse_node(input, path);
      ValueSpec value;
      value.id = static_cast<ValueId>(data.values.size());
      value.name = input_node.name;
      value.required_bytes = input_node.bytes;
      merge_node_evidence(value, input_node, path);
    ...
    for (const auto& ordered_plugin : ordered) {
      ...
      for (std::size_t index = 0; index < output_nodes.size(); ++index) {
        ...
        merge_node_evidence(value, output_nodes[index],
                            path + ".output_nodes[" + std::to_string(index) + "]");
        if (op.kind == OpKind::Mla && value.logical_dtype.has_value() &&
            value.logical_shape.has_value() &&
            value.logical_layout == std::optional<std::string>{"normal"}) {
          const auto expected = dense_bytes(*value.logical_shape, *value.logical_dtype);
          if (expected.has_value() && *expected == value.required_bytes) {
            value.representation = ValueRepresentation::Dense;
          }
        }
    ...
    for (std::size_t index = 0; index < mla.inputs.size(); ++index) {
      ...
      if (physical_extent != value.required_bytes) {
        reject(LegacyAfeDecodeErrorCode::ValueSizeMismatch,
               ...
               "QMLA IFM physical extent does not exactly equal the MPK logical tensor");
      }
      ...
      result.proof.push_back({"MLA.IFM[" + std::to_string(index) + "]",
                              "ELF symbol '" + symbol + "' has exact physical extent " +
                                  std::to_string(physical_extent) +
                                  " bytes and MPK MLA input[" + std::to_string(index) +
    ...
    for (std::size_t index = 0; index < mla.outputs.size(); ++index) {
      ...
      author_mla_output_storage_binding(data, mla_op_index, pass_op_index, index, symbol,
                                        physical_extent, &result.proof);
      ...
      result.proof.push_back({"MLA.OFM[" + std::to_string(index) + "]",
                              "ELF symbol '" + symbol + "' has exact physical extent " +
                                  std::to_string(physical_extent) +
                                  " bytes and MPK MLA output[" + std::to_string(index) +
                                  "] agree; 4096-byte alignment is explicit legacy CMA policy"});
    }
