...
FrameSlotArenaPlan::compile(const ModelExecutionPlan& execution_plan,
  ...
  std::vector<std::uint64_t> physical_extent(values.size(), 0U);
  for (const auto& value : values) {
    physical_extent[value.id] = value.required_bytes;
  }
  ...
  for (const auto& port : execution_plan.backend_ports()) {
    ...
    physical_extent[root] =
        std::max(physical_extent[root], port.physical_extent_bytes);
  }
  ...
  for (const auto& value : values) {
    if (is_model_input[value.id] || !is_materialized(value)) {
      continue;
    }
    ...
    region.size_bytes = physical_extent[value.id];
