...
bool validate_read_expression(const ModelExecutionPlanData& data, const ValueSpec& value,
  ...
  const bool self_backed = expression.source_value_id == value.id;
  if (expression.source_value_id >= data.values.size() ||
      (!self_backed && expression.source_value_id >= value.id)) {
  ...
  if (self_backed && expression.byte_offset != 0U) {
    return fail(error, "execution-plan materialized storage binding has a non-zero base offset");
  }
  if (!self_backed && source.read_expression.has_value() &&
      source.read_expression->source_value_id != source.id) {
  ...
  // A legal affine view is positive, row-major monotonic, and cannot overlap
  // adjacent logical index ranges.  Larger strides are exact padding; smaller
  // strides would alias logical elements and are never accepted as layout.
  std::uint64_t physical_span = element_bytes;
  for (std::size_t reverse = expression.stride_bytes.size(); reverse > 0U; --reverse) {
    ...
    if (static_cast<std::uint64_t>(stride) < physical_span) {
      return fail(error, "execution-plan read expression has overlapping affine strides");
    }
    ...
    if (!checked_mul(static_cast<std::uint64_t>((*value.logical_shape)[axis] - 1),
        ...
        !checked_add(axis_span, physical_span, &physical_span)) {
      return fail(error, "execution-plan read expression span overflows");
    }
  }

  std::uint64_t carrier_extent = source.required_bytes;
  for (const auto& port : data.backend_ports) {
    if (port.value_id == source.id) {
      carrier_extent = std::max(carrier_extent, port.physical_extent_bytes);
    }
  }
  std::uint64_t end = 0U;
  if (!checked_add(expression.byte_offset, physical_span, &end) || end > carrier_extent) {
...
bool validate(const ModelExecutionPlanData& data, std::string* error) {
  ...
  std::vector<std::uint64_t> backend_storage_extents(data.values.size(), 0U);
  ...
  for (const auto& port : data.backend_ports) {
    ...
    auto& storage_extent = backend_storage_extents[port.value_id];
    if (storage_extent != 0U && storage_extent != port.physical_extent_bytes) {
      return fail(error, "execution-plan backend ports contradict one storage physical span");
    }
    storage_extent = port.physical_extent_bytes;
