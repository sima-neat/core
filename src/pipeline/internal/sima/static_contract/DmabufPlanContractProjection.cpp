...
bool is_materialized(const ValueSpec& value) {
  return !value.read_expression.has_value() ||
         value.read_expression->source_value_id == value.id;
}
...
const ValueSpec* find_materialized_value_for_logical(
  ...
  for (const auto* name :
    ...
    for (const auto& value : plan.values()) {
      if (!is_materialized(value) || value.name != *name) {
        continue;
      }
      if (result) {
...
std::optional<CvuOutputPlacement> resolve_cvu_output_placement(
  ...
  if (!value || !is_materialized(*value)) {
...
read_views_consumed_after_mla(const ModelExecutionPlan& plan, const ValueId root_value_id) {
  ...
  // Public publication aliases are the authoritative names/order for a
  // backend output. Prefer them over the internal consumer edge which feeds
  // PassThrough; both carry the same root-relative affine expression.
  for (const auto& output : plan.model_outputs()) {
    const auto* value = plan.value(output.value_id);
    if (!value || !value->read_expression.has_value() ||
        value->read_expression->source_value_id != root_value_id ||
        !seen.emplace(value->id).second) {
      continue;
    }
  ...
  if (!packed_read_views.empty()) {
    ...
    for (std::size_t index = 0; index < outputs.size(); ++index) {
      ...
      if (value->read_expression.has_value()) {
        logical.stride_bytes = value->read_expression->stride_bytes;
      }
      contract->logical_outputs.push_back(std::move(logical));
    }
  }
