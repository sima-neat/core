...
std::uint64_t dtype_size_bytes_from_token(const std::string& raw_dtype) {
  ...
  if (token.find("BF16") != std::string::npos || token.find("FLOAT16") != std::string::npos ||
      token.find("FP16") != std::string::npos ||
      token == "INT16" || token == "EVXX_INT16" || token == "UINT16") {
    return 2U;
  }
...
LogicalInputStaticSpec build_logical_input_static_spec(
    ...
    std::uint64_t size_bytes_override,
  ...
  logical.size_bytes =
      size_bytes_override > 0U ? size_bytes_override : tensor_size_bytes_from_shape_dtype(shape, dtype);
  logical.dtype = dtype;
...
LogicalTensorStaticSpec build_logical_output_static_spec(
  ...
  if (detess_layout_debug_enabled_local()) {
    std::fprintf(
        stderr,
        "[detess-layout-debug] where=specbuilder.build_logical_output logical=%d backend=%d physical=%d "
...
PhysicalBufferStaticSpec build_physical_buffer_static_spec(int physical_index,
                                                           ...
                                                           const std::string& segment_name,
                                                           int source_physical_index,
                                                           std::int64_t source_byte_offset) {
...
InputBindingStaticSpec build_input_binding_static_spec(int sink_pad_index,
                                                       ...
                                                       int src_logical_output_index,
                                                       int src_output_slot,
                                                       int src_physical_output_index,
                                                       std::uint64_t src_physical_size_bytes,
                                                       std::int64_t src_physical_byte_offset,
                                                       bool required) {
...
void finalize_logical_input_spec(LogicalInputStaticSpec* logical,
  ...
  if (logical->logical_name.empty()) {
    logical->logical_name = !logical->backend_name.empty()
  ...
  if (logical->backend_name.empty()) {
    logical->backend_name = !logical->logical_name.empty() ? logical->logical_name
                                                           : logical->segment_name;
  }
...
void finalize_logical_output_spec(LogicalTensorStaticSpec* logical,
  ...
  if (detess_layout_debug_enabled_local()) {
    std::fprintf(
        stderr,
        "[detess-layout-debug] where=specbuilder.finalize_logical_output index=%zu logical=%d physical=%d "
