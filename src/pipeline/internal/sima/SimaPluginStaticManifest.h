...
struct LogicalInputStaticSpec {
  ...
  TensorMaterializationKind materialization_kind = TensorMaterializationKind::Direct;  ///< Materialization kind.
  std::optional<QuantStaticSpec> quant;  ///< Quant params, if quantized.
};
...
/// Placeholder payload for quant / dequant / tess / quanttess stages (carries only a reserved field today).
