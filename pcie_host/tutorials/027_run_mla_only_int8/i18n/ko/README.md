...
애플리케이션이 양자화를 소유할 때 `mla_only`를 활성화하세요. 센서나 앞단 모델에서 이미 INT8을 생성하는 경우, 자체 후처리를 위해 원시 INT8 헤드가 필요한 경우, 또는 카드 측 지연 시간에서 양자화와 역양자화 단계를 제거하려는 경우입니다. 모든 스케일과 영점은 `model.info()`에서 읽고, 모델의 다른 빌드에서 복사하지 마세요.
