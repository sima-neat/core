...
void require_forced_preproc_options_complete(const simaai::neat::Model::Options& opt,
  ...
  auto fail_missing = [&](const std::string& field, const std::string& reason) {
    throw std::runtime_error(
        "graph_migration_legacy_preproc_yolov8_matrix_test: missing required option '" + field +
...
AccuracyResult run_boxdecode_accuracy(simaai::neat::Model& model, const cv::Mat& input_frame,
  ...
  try {
    const simaai::neat::Tensor input_tensor = make_model_input_tensor(
        input_frame, model, opt, "graph_migration_legacy_preproc_yolov8_matrix_test:accuracy");
...
int main(int argc, char** argv) {
  ...
  (void)argc;
  (void)argv;
  return skip_long_test("OpenCV required for graph_migration_legacy_preproc_yolov8_matrix_test");
#else
  try {
    ...
    std::cout << "[OK] graph_migration_legacy_preproc_yolov8_matrix_test passed models="
              << run_count << "\n";
    return 0;
