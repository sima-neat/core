...
RUN_TEST("graph_migration_phase3_negative_diagnostics_test", [] {
...

  {
    ...
    auto source0 = live_camera_source_fragment("compiler_merge_outer_override_camera0");
    auto source1 = live_camera_source_fragment("compiler_merge_outer_override_camera1");
    auto sink = linked_passthrough_fragment("image", "classes", inner);
    simaai::neat::Graph app;
    app.connect(source0, sink, outer);
    app.connect(source1, sink, outer);
