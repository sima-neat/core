...
RUN_TEST("graph_migration_phaseA4_graph_run_export_test", [] {
  ...
  opt.metadata.push_back(std::make_pair(
      std::string("test_name"), std::string("graph_migration_phaseA4_graph_run_export_test")));

  std::string err;
  const std::string body = graph_run_to_json(run, opt, &err);
  require(err.empty(), "graph_run_to_json error: " + err);
  require(!body.empty(), "graph_run_to_json returned empty body");
  ...
  const std::filesystem::path path = tmp_path("graph_run_export.neat.graph_run.json");
  std::filesystem::remove(path);
  require(save_graph_run_json(run, path.string(), opt, &err), "save_graph_run_json failed: " + err);
  require(std::filesystem::exists(path), "save_graph_run_json did not create output file");
  ...
  build_opt.graph_run_export.path = auto_path.string();
  build_opt.graph_run_export.label = "auto_export";
