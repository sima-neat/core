...
struct Lib {
  ...
  simaai_memory_t* (*memcpy_part)(simaai_memory_t*, uint64_t, simaai_memory_t*, uint64_t,
                                  uint64_t) = nullptr;
  bool ok() const {
    return alloc_flags && map && invalidate && flush && memcpy_part && freev;
  }
};
...
Lib load() {
  ...
  if (!h)
    h = dlopen("libsimaaimem.so", RTLD_LAZY | RTLD_LOCAL);
  if (!h)
    return l;
  ...
  l.invalidate_part =
      reinterpret_cast<decltype(l.invalidate_part)>(S("simaai_memory_invalidate_cache_part"));
...
struct SimaaiCmo : CmoBackend {
  ...
  void invalidate(void*, uint64_t, uint64_t) override {
    lib->invalidate(mem);
  }
  void clean(void*, uint64_t, uint64_t) override {
    lib->flush(mem);
  }
};
...
struct Buf {
  Lib* lib = nullptr;
  simaai_memory_t* mem = nullptr;
  volatile uint8_t* cpu = nullptr;
  uint64_t size = 0;
...
struct TF : std::runtime_error {
  explicit TF(const std::string& m) : std::runtime_error(m) {}
};
...
struct Rig {
  ...
  Rig(Lib* l, uint64_t n) : lib(l), coh(nullptr, n), be(l, nullptr) {
    if (!T.alloc(l, n) || !S.alloc(l, n) || !V.alloc(l, n))
  ...
  void cpu_read_T(uint64_t off, uint64_t len, const std::vector<uint8_t>& expect,
                  const char* where) {
    begin_cpu_access(coh, Access::Read, &be);
    int bad = -1;
    uint8_t g = 0;
    for (uint64_t i = 0; i < len; ++i) {
      if (T.cpu[off + i] != expect[i]) {
        bad = (int)i;
        g = T.cpu[off + i];
        break;
      }
    ...
    if (bad >= 0)
      throw std::runtime_error(std::string(where) + ": stale CPU read at byte " +
                               std::to_string(off + bad) + " got " + std::to_string(g) + " want " +
                               std::to_string(expect[bad]));
  }
};

struct Case {
  const char* id;
  std::function<void(Lib&)> fn;
};
std::vector<Case>& reg() {
...
struct Reg {
  Reg(const char* id, std::function<void(Lib&)> f) {
    reg().push_back({id, std::move(f)});
  }
};
#define HWTEST(id)                                                                                 \
  static void id(Lib&);                                                                            \
  static Reg r_##id(#id, id);                                                                      \
  static void id(Lib& lib)

std::vector<uint8_t> pat(uint64_t n, uint8_t s) {
  std::vector<uint8_t> v(n);
  for (uint64_t i = 0; i < n; ++i)
    v[i] = uint8_t(s + i * 31 + (i >> 3));
  return v;
}
...
HWTEST(P0_L2_12_cpu_to_device_lost_write) {
  ...
  for (uint64_t i = 0; i < 256; ++i)
    check(got[i] == v[i], "lost write: device missed CPU store");
}
...
HWTEST(P0_L2_13_round_trip) {
  ...
  for (int it = 0; it < 500; ++it) {
    ...
    for (uint64_t i = 0; i < 256; ++i)
      r.V.cpu[i] = uint8_t(r.V.cpu[i] + 1); // transform
    lib.flush(r.V.mem);
...
HWTEST(P0_L2_16_multi_region) {
  Rig r(&lib, 4096);
  for (int q = 0; q < 4; ++q)
    r.dev_write(q * 256, pat(256, uint8_t(q * 7 + 1))); // OLD
  begin_cpu_access(r.coh, Access::Read, &r.be);         // prime cache
  for (int q = 0; q < 4; ++q) {
    volatile uint8_t t = r.T.cpu[q * 256];
    (void)t;
  }
  end_cpu_access(r.coh, Access::Read, &r.be);
  for (int q = 0; q < 4; ++q)
    r.dev_write(q * 256, pat(256, uint8_t(q * 11 + 3))); // NEW
...
HWTEST(P0_L4_24_producer_consumer_handoff) {
  ...
  for (int i = 1; i <= kFrames; ++i) {
    while (turn.load(std::memory_order_acquire) != 1)
      std::this_thread::yield();
    try {
      r.cpu_read_T(0, 256, pat(256, uint8_t(i)), "handoff");
    } catch (...) {
      ++bad;
    }
...
HWTEST(P0_L5_29_real_ddr_fuzzer) {
  ...
  for (uint64_t n = 0; n < ops; ++n) {
    ...
    case 1: { // CPU read, checked
      ...
      end_cpu_access(r.coh, Access::Read, &r.be);
      if (bad >= 0)
        throw TF("fuzz cpu-read stale @op " + std::to_string(n) + " byte " +
                 std::to_string(off + bad) + " got " + std::to_string(g) + " want " +
                 std::to_string(oracle[off + bad]));
      break;
    }
    case 2: // device write
...
int main(int argc, char** argv) {
  if (argc > 1)
    g_fuzz_ops = strtoull(argv[1], nullptr, 10); // optional fuzzer op count
  Lib lib = load();
  if (!lib.ok()) {
    std::printf("[SKIP] libsimaaimem not usable\n");
    return kSkip;
  }
  ...
  for (const auto& c : reg()) {
    try {
      ...
      if (std::string(e.what()) == "skip") {
        std::printf("[SKIP] %s (alloc failed)\n", c.id);
        ++skipped;
      } else {
        std::printf("[FAIL] %s: %s\n", c.id, e.what());
        ++failures;
