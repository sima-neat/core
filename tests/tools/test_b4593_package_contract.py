...
printf '%s|%s|%s\n' \
  "${{B4593_KERNEL_PACKAGE_VERSION}}" \
  "${{B4593_LIBCAMERA_VERSION}}" \
  "${{B4593_MEMORY_VERSION}}"
"""
        )

        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("6.18.3-4593|2.1.3+neat2|2.1.1-0neat5", result.stdout)
...
"""
        ...
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("VERSION=2.1.3+neat2", result.stdout)
...
touch "${deb}"
printf '%s\n' \
  'Inst libcamera [2.1.3~pre4593] (2.1.3+neat2 local [arm64])' \
