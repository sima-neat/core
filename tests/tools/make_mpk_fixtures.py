#!/usr/bin/env python3
"""Generate deterministic MPK fixtures and checksum manifest."""
...
def _write_tar(entries: Iterable[dict], out_path: Path) -> None:
    ...
    # Use deterministic gzip headers for .mpk/.tgz/.tar.gz.
...
def _fixture_specs() -> List[FixtureSpec]:
    ...
    return [
        FixtureSpec("valid/basic_valid.tar", "baseline valid package (tar)", valid),
        FixtureSpec("valid/basic_valid.mpk", "baseline valid package (mpk/gz)", valid),
        FixtureSpec("valid/multi_stage_valid.mpk", "valid package with multi-stage inference", multi),
        FixtureSpec("invalid/missing_pipeline_sequence.mpk", "missing required pipeline_sequence.json", missing_pipeline),
        FixtureSpec("invalid/schema_error_sequence.mpk", "invalid JSON in pipeline_sequence.json", schema_error),
        FixtureSpec("invalid/unsupported_version.mpk", "manifest version not supported", unsupported_version),
        ...
        FixtureSpec("invalid/hardlink_escape.mpk", "hardlink escape entry", hardlink_escape),
        FixtureSpec("invalid/oversized_entry.mpk", "oversized entry for size-limit checks", oversized_entry),
        FixtureSpec("invalid/unicode_traversal_fullwidth_slash.mpk", "unicode fullwidth slash traversal", unicode_traversal_fullwidth_slash),
        FixtureSpec("invalid/unicode_traversal_division_slash.mpk", "unicode division slash traversal", unicode_traversal_division_slash),
        FixtureSpec("invalid/unicode_traversal_fullwidth_backslash.mpk", "unicode fullwidth backslash traversal", unicode_traversal_fullwidth_backslash),
        FixtureSpec("invalid/unicode_confusable_dotdot.mpk", "unicode confusable dot traversal", unicode_confusable_dotdot),
        FixtureSpec("invalid/duplicate_header_same_path.mpk", "duplicate tar header for same path", duplicate_header_same_path),
        FixtureSpec("invalid/duplicate_header_safe_then_traversal.mpk", "duplicate header using alternate raw path form", duplicate_header_safe_then_traversal),
        FixtureSpec("invalid/json_deep_nesting_256.mpk", "pipeline_sequence exceeds JSON depth limit", json_deep_nesting_256),
        FixtureSpec("invalid/json_duplicate_keys_top.mpk", "duplicate keys in top-level pipeline_sequence object", json_duplicate_keys_top),
        FixtureSpec("invalid/json_duplicate_keys_nested.mpk", "duplicate keys in nested stage entry", json_duplicate_keys_nested),
        FixtureSpec("invalid/json_huge_numeric_sequence_id.mpk", "oversized numeric field in sequence entry", json_huge_numeric_sequence_id),
    ]
...
def _build_fixture_tree(out_root: Path) -> Dict[str, dict]:
    ...
    trunc_rel = "invalid/truncated_archive.mpk"
    src = out_root / "valid/basic_valid.mpk"
    ...
    # Header/path mutation fixtures derived from baseline valid package.
    tar_bytes = bytearray((out_root / "valid/basic_valid.tar").read_bytes())
    offsets = _header_offsets(tar_bytes)
    if not offsets:
        raise RuntimeError("failed to parse headers from valid/basic_valid.tar")

    # invalid/non_utf8_path_bytes.mpk
    non_utf8_rel = "invalid/non_utf8_path_bytes.mpk"
    ...
    checksum_header_rel = "invalid/checksum_corrupt_header.mpk"
    ...
    # invalid/checksum_corrupt_body.mpk (gzip trailer CRC corruption)
    checksum_body_rel = "invalid/checksum_corrupt_body.mpk"
    checksum_body_path = out_root / checksum_body_rel
    checksum_body = bytearray((out_root / "valid/basic_valid.mpk").read_bytes())
    if len(checksum_body) < 8:
        raise RuntimeError("valid/basic_valid.mpk too small for gzip CRC mutation")
...
def _manifest_payload(generated: Dict[str, dict]) -> dict:
    ...
    return {
        "format_version": 1,
        "generator": "tests/tools/make_mpk_fixtures.py",
        "fixtures": fixtures,
    }
...
def _check(root: Path, manifest_path: Path) -> int:
    with tempfile.TemporaryDirectory(prefix="mpk_fixtures_check_") as td:
    ...
    print("MPK fixture check passed.")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description="Generate deterministic MPK fixtures")
    parser.add_argument("--root", default="tests/assets/mpk", help="fixture root directory")
    parser.add_argument(
        "--manifest",
        default="tests/assets/mpk/fixtures_manifest.json",
