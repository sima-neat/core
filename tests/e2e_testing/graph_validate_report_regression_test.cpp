...
RUN_TEST(
    "graph_validate_report_regression_test", ([] {
...

      {
        ...
        const GraphReport rep = empty.validate(opt);
...

      {
        ...
        ValidateOptions opt;
        opt.parse_launch = false;
        const GraphReport rep = source_only.validate(opt);
...

      {
        ...
        ValidateOptions opt;
        opt.parse_launch = false;
        const GraphReport rep = with_input.validate(opt);
        require(rep.error_code == error_codes::kPipelineShape,
      ...
      // Live source validate() should time out in PAUSED and report deterministically.
      {
        ...
        ValidateOptions opt;
        opt.parse_launch = true;
        const GraphReport rep = live_source.validate(opt);
      ...
      // Finite source validate() should populate report fields on parse path.
      {
        ...
        ValidateOptions opt;
        opt.parse_launch = true;
        const GraphReport rep = finite_source.validate(opt);
