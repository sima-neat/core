...
int main() {
  try {
      ...
      if (sample) {
        ...
        if (!checked_caps) {
          ...
          if (!caps) {
            ...
            throw std::runtime_error("appsink sample missing caps");
          }
    ...
    if (!checked_caps) {
      throw std::runtime_error("did not observe output caps");
    }
    ...
    if (const char* env = std::getenv("SIMA_DECODER_MIN_FRAMES")) {
      ...
      if (errno == 0 && end != env && *end == '\0' && parsed > 0) {
        min_frames = static_cast<int64_t>(parsed);
      }
    }
    if (frames < min_frames) {
      std::ostringstream oss;
      oss << "frame count too low: got " << frames << " expected >= " << min_frames;
      throw std::runtime_error(oss.str());
    }

    std::cout << "[OK] decoder_visible_caps_test passed (" << frames << " frames)\n";
    return 0;
