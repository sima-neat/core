...
RUN_TEST("stress_session_validate_rtsp_churn_test", ([] {
           ...
           for (int i = 0; i < validate_cycles; ++i) {
             ...
             SessionReport rep;
             try {
               rep = session.validate(vopt, frame);
             ...
             require(
                 !rep.pipeline_string.empty(),
                 "stress_session_validate_rtsp_churn: empty pipeline string in validate report");
             require(!rep.repro_note.empty(),
                     "stress_session_validate_rtsp_churn: empty repro note in validate report");
           }
           ...
           for (int i = 0; i < rtsp_cycles; ++i) {
             ...
             rtsp_session.add(nodes::StillImageInput(image.string(), 640, 480, 640, 480, 10));
             ...
             try {
               handle = rtsp_session.run_rtsp(ropt);
             } catch (const std::exception& e) {
               if (sima_test::likely_runtime_missing(e.what())) {
             ...
             require(handle.running(),
                     "stress_session_validate_rtsp_churn: RTSP handle not running");
             std::this_thread::sleep_for(std::chrono::milliseconds(60));
             handle.stop();
             handle.stop();
             require(!handle.running(),
                     "stress_session_validate_rtsp_churn: RTSP handle should be stopped");
           }
         }));
