...
inline fs::path model_archive_fixture_root() {
  return sima_test::test_model_archive_fixture_root_path();
}

inline fs::path model_archive_fixture_path(const std::string& rel) {
  return model_archive_fixture_root() / rel;
}

inline std::string make_temp_dir(const std::string& tag) {
  fs::path base = fs::temp_directory_path() / "sima_neat_model_archive_tests";
  std::error_code ec;
  ...
  if (!out) {
    throw std::runtime_error("model_archive_test_utils: mkdtemp failed");
  }
