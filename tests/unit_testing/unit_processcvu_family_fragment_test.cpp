...
nlohmann::json make_single_io_cfg(const char* graph_name) {
  return nlohmann::json{
      {"graph_name", graph_name}, {"input_width", 640},   {"input_height", 640}, {"input_depth", 3},
      {"output_width", 640},      {"output_height", 640}, {"output_depth", 3},
  };
}
...
RUN_TEST("unit_processcvu_family_fragment_test_quanttess", ([] {
           ...
           require_contains(frag, "neatprocesscvu name=n4_quanttess",
           ...
           require_not_contains(frag,
                                "config=", "QuantTess fragment must not emit legacy config path");
         }));
...
RUN_TEST("unit_processcvu_family_fragment_test_detessdequant", ([] {
           ...
           require_not_contains(
               frag, "config=", "DetessDequant fragment must not emit legacy config path");
         }));
