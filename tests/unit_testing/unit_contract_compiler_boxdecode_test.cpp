...
RUN_TEST("unit_contract_compiler_boxdecode_test", ([] {
  ...
  contract.tensors = {
      BoxDecodeTensorStaticContract{{80, 80, 64}, {80, 80, 64}, "INT8", "HWC",
                                    "reg_head0", "reg_head0", "reg_head0", 0, 0, -1, 0, 409600},
      BoxDecodeTensorStaticContract{{80, 80, 80}, {80, 80, 80}, "INT8", "HWC",
                                    "cls_head0", "cls_head0", "cls_head0", 1, 1, -1, 0, 512000},
  };
  ...
  packed_contract.tensors = {
      BoxDecodeTensorStaticContract{{80, 80, 64}, {80, 80, 64}, "INT8", "HWC",
                                    "reg_head0", "reg_head0", "reg_head0", 0, 0, 0, 128, 409600},
  };
  ...
  require(packed_compiled.runtime_contract.logical_inputs.front().segment_name == "output_tensor",
          "packed boxdecode logical input should use the physical TensorBuffer parent segment");
  require(packed_compiled.runtime_contract.input_bindings.front().source_segment_name == "output_tensor",
          "packed boxdecode input binding should source the packed parent segment");
  require(packed_compiled.runtime_contract.input_bindings.front().src_physical_output_index == 0,
          "packed boxdecode input binding should preserve physical output index");
  require(packed_compiled.runtime_contract.input_bindings.front().src_physical_byte_offset == 128,
  ...
  const auto finalized_contract = finalize_boxdecode_static_contract(
      packed_contract, BoxDecodeType::YoloV8, std::nullopt, route_flags,
      BoxDecodeTypeOption::GroupedByRoleLogit, 0.25, 0.55, 100, 0,
  ...
  require(finalized_compiled.payload.decode_type_option == BoxDecodeTypeOption::GroupedByRoleLogit,
