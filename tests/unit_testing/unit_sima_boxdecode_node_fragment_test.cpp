...
RUN_TEST(
    "unit_sima_boxdecode_node_fragment_test", ([] {
      ...
      require(managed_fragment.starts_with("neatobjectdecode "),
              "model-managed BoxDecode must use the production neatobjectdecode factory");
  ...
  "input_buffers": [{"name": "decoder"}]
      ...
      simaai::neat::CompiledNodeContract compiled_boxdecode;
      std::string compile_error;
      require(
          managed_box->compile_node_contract(compile_input, &compiled_boxdecode, &compile_error),
          "model-managed neatobjectdecode contract must compile: " + compile_error);
      require(compiled_boxdecode.boxdecode.has_value(),
              "model-managed neatobjectdecode must retain its typed contract");
      for (const auto& logical : compiled_boxdecode.boxdecode->runtime_contract.logical_inputs) {
        require(logical.shape.size() == 4U && logical.shape.front() == 1,
                "batch-1 model-managed neatobjectdecode inputs must expose explicit N");
      }

      GstElementFactory* production_feature = gst_element_factory_find("neatobjectdecode");
      require(production_feature != nullptr,
              "matching Internals test runtime must provide neatobjectdecode");
      gst_registry_remove_feature(gst_registry_get(), GST_PLUGIN_FEATURE(production_feature));
      bool missing_factory_rejected = false;
      try {
        (void)simaai::neat::nodes::SimaBoxDecode(simaai::neat::BoxDecodeType::YoloV8, 0.25, 0.45,
                                                 100, "missing_production_decoder");
      } catch (const std::exception& error) {
        const std::string message = error.what();
        missing_factory_rejected =
            message.find("required GStreamer element 'neatobjectdecode' is not available") !=
                std::string::npos &&
            message.find("matching NEAT Internals package") != std::string::npos &&
            message.find("active GStreamer plugin path") != std::string::npos;
      }
      require(missing_factory_rejected,
              "a missing production BoxDecode plugin must fail with installation guidance");
      gst_object_unref(production_feature);
    }));
