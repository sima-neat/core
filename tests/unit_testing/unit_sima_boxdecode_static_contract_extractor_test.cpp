...
RUN_TEST(
    "unit_sima_boxdecode_static_contract_extractor_test", ([] {
      ...
      c16_mla.kernel = "mla";
      c16_mla.canonical_output_dtype = "INT8";
      ...
      for (std::size_t i = 0; i < c16_names.size(); ++i) {
        c16_unpack.output_tensors.push_back(MpkTensorContract{
            ...
            .shape_semantics = MpkShapeSemantics::Geometry,
            .size_bytes =
                static_cast<std::size_t>(c16_widths[i] * c16_widths[i] * c16_logical_channels[i]),
            .byte_offset = 0,
      ...
      require(extracted_c16_padded.has_value(),
              "c16 padded MLA route should resolve a boxdecode contract: " + error);
      ...
      require(extracted_c16_padded->tensors.size() == 6U,
              "c16 padded route should preserve all YOLO heads");
      require(extracted_c16_padded->tensors[0].input_shape == std::vector<int>({80, 80, 16}) &&
              ...
              "c16 padded bbox head should expose physical C16 input and logical slice");
      ...
      require(c16_expected_total == 806400U,
              "boxdecode C16 packed view sizes should add up to the raw MLA buffer size");
      require(extracted_c16_padded->physical_inputs[0].size_bytes == 102400U,
              "first bbox physical binding should use padded 80x80x16 INT8 transport bytes");
