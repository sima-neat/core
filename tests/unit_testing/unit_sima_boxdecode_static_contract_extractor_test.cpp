...
RUN_TEST("unit_sima_boxdecode_static_contract_extractor_test", ([] {
           using namespace simaai::neat::pipeline_internal::sima;
           using namespace simaai::neat::pipeline_internal::sima::plugin_contracts;

           auto shape_desc_matches = [](const sima_ev_shape_desc& desc,
           ...
           auto require_subset_matches_static_contract =
               [shape_desc_matches](const BoxDecodeContractSubset& subset,
                 ...
                 if (contract.decode_type_option == simaai::neat::BoxDecodeTypeOption::Auto) {
                   require(!subset.decode_type_option.has_value(),
                           context + ": decode type option should stay unset when the extracted contract leaves it auto");
                 } else {
                 ...
                 for (std::size_t i = 0; i < contract.tensors.size(); ++i) {
                   ...
                   const std::string expected_logical_name =
                       !tensor.logical_name.empty()
                           ? tensor.logical_name
                           : (i < contract.tensor_names.size() ? contract.tensor_names[i]
                   ...
                   require(logical.dtype == (!tensor.data_type.empty() ? tensor.data_type
                                                                       : contract.input_dtype),
                   ...
                   require(binding.source_segment_name ==
                               (!contract.physical_inputs.empty() &&
                   ...
                   if (contract.quant_needed) {
                     ...
                     require(logical.quant->zero_points ==
                                 std::vector<std::int64_t>{contract.dq_zp[i]},
           ...
           std::string error;
           const auto extracted = build_boxdecode_static_contract_from_mpk(
               mpk, make_flags(true, false), &error);
           require(extracted.has_value(),
                   "mpk boxdecode extraction should preserve per-head source facts: " + error);
           const auto extracted_subset = extract_boxdecode_contract_subset_from_mpk(
               mpk, make_flags(true, false), nullptr, &error);
           require(extracted_subset.has_value(),
                   "mpk boxdecode subset extraction should preserve per-head source facts: " +
                       error);
           ...
           require(extracted->tensors[0].source_segment_name == "MLA_0",
                   "boxdecode extractor should preserve the upstream MLA parent segment for runtime binding");
           ...
           require(extracted->physical_inputs[0].name == "MLA_0",
           ...
           const auto extracted_missing_detess = build_boxdecode_static_contract_from_mpk(
               mpk, make_flags(true, true), &error);
           require(!extracted_missing_detess.has_value(),
                   "external boxdecode should hard-fail when route flags advertise tess but no upstream detess slice facts exist");
           ...
           const std::array<const char*, 6> semantic_names = {
               "opaque_regression_head_a",
               "opaque_regression_head_b",
               "opaque_regression_head_c",
               "opaque_scores_head_a",
               "opaque_scores_head_b",
               "opaque_scores_head_c",
           };
           ...
           const auto extracted_decoupled = build_boxdecode_static_contract_from_mpk(
               decoupled_mpk, make_flags(true, true), &error);
           ...
           require_subset_matches_static_contract(*extracted_decoupled_subset,
                                                  *extracted_decoupled,
           ...
           require(extracted_decoupled->tess_needed,
                   "external/raw unpack route must preserve downstream detess lineage in the typed route facts");
           ...
           require(extracted_decoupled->dq_scale.size() == 6U &&
                       extracted_decoupled->dq_scale[0] == 1.0 &&
                       extracted_decoupled->dq_scale[1] == 2.0 &&
                       extracted_decoupled->dq_scale[2] == 3.0 &&
                       extracted_decoupled->dq_scale[3] == 4.0 &&
                       extracted_decoupled->dq_scale[4] == 5.0 &&
                       extracted_decoupled->dq_scale[5] == 6.0,
                   "decoupled route should preserve per-branch quant scales in extracted order");
           require(extracted_decoupled->dq_zp.size() == 6U &&
                       extracted_decoupled->dq_zp[0] == 11 &&
                       extracted_decoupled->dq_zp[1] == 12 &&
                       extracted_decoupled->dq_zp[2] == 13 &&
                       extracted_decoupled->dq_zp[3] == 14 &&
                       extracted_decoupled->dq_zp[4] == 15 &&
           ...
           require(extracted_probability_quant.has_value(),
           ...
           require(extracted_probability_quant_subset.has_value(),
                   "probability-domain quantized YOLO subset extraction should preserve typed route facts: " +
                       error);
           ...
           require(extracted_probability_quant->score_activation == BoxDecodeScoreActivation::Identity,
                   "core extractor should infer probability-domain score activation generically from quant range semantics");
           ...
           require(extracted_packed_parent_subset.has_value(),
                   "packed-parent MLA subset extraction should preserve both logical and physical source facts: " +
                       error);
           require_subset_matches_static_contract(*extracted_packed_parent_subset,
                                                  *extracted_packed_parent,
                                                  "packed-parent MLA route");
           ...
           require(extracted_packed_parent->physical_inputs[0].name == "output_tensor",
                   "explicit unpack boundary should publish the packed MLA parent as the physical input name");
           ...
           require(extracted_packed_parent_bypass.has_value(),
                   "raw-parent bypass should allow external boxdecode to source directly from packed MLA parent: " +
                       error);
