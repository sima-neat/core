...
RUN_TEST("unit_sample_tensor_set_materialized_parent_offsets_test", ([] {
           ensure_gst_ready();
           ...
           require(view.tensors[0].byte_offset == 0U,
                   "first tensor should start at offset zero");
           ...
           err.clear();
           auto joined_holder =
               pipeline_internal::make_sample_holder_from_bundle(joined_sample, &err);
           ...
           require(joined_gst_sample != nullptr,
                   "joined-parent holder should be a GstSample");
           ...
           require(joined_caps_struct != nullptr,
                   "joined-parent holder caps should have a structure");
