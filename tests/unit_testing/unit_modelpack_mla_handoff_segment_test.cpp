...
RUN_TEST(
    "unit_modelpack_mla_handoff_segment_test", ([] {
      ...
      const auto preproc_handoff =
          pipeline_internal::sima::resolve_processcvu_single_handoff_output(
      ...
      for (std::size_t i = 0; i < post_plan.size(); ++i) {
        ...
        const auto rebuilt_post = pipeline_internal::sima::stagesemantics::
            build_processcvu_mpk_compiled_contract_for_stage_kind(*pack.mpk_contract(),
                                                                  post_plan[i].kind);
        ...
        require(
            rebuilt_post.runtime_contract.logical_outputs.size() ==
      ...
      for (const auto& bf16_path : bf16_candidates) {
        ...
        const auto pre_stage_facts =
            bf16_pack.stage_facts_for_model_stage(internal::ModelStage::Preprocess);
        ...
        const auto rebuilt_tess = pipeline_internal::sima::stagesemantics::
        ...
        require(canonical_tess.exposed_view.primary_output_name == "output_tessellated_image",
                "BF16 tensor route canonical tess contract should publish the graph-owned handoff "
                "name");
        ...
        require(canonical_tess.runtime_contract.physical_outputs.front().segment_name ==
                ...
                "BF16 tensor route canonical tess physical output should preserve the MLA handoff "
                "segment");
        const auto handoff =
        ...
        require(handoff->physical_index ==
        ...
        require(
            tess_opt.compiled_contract->runtime_contract.physical_outputs.front().segment_name ==
                canonical_tess.runtime_contract.physical_outputs.front().segment_name,
            "TessOptions(Model) should reuse the canonical tess physical output segment");

        const auto infer_stage_facts =
            bf16_pack.stage_facts_for_model_stage(internal::ModelStage::MlaOnly);
        const internal::ModelFragment::StageFacts* mla_stage_fact = nullptr;
        for (const auto& fact : infer_stage_facts) {
          if (fact.mla_compiled.has_value()) {
            mla_stage_fact = &fact;
        ...
        require(
            mla_stage_fact->mla_compiled->runtime_contract.physical_inputs.front().segment_name ==
                "output_tessellated_image",
            "BF16 tensor route MLA input should match the canonical tess handoff segment");
        require(
            mla_stage_fact->mla_compiled->runtime_contract.logical_inputs.front().size_bytes ==
                mla_stage_fact->mla_compiled->runtime_contract.physical_inputs.front().size_bytes,
            "BF16 tensor route MLA logical input should preserve the packed tess handoff byte "
            "size");
        require(
            mla_stage_fact->mla_compiled->runtime_contract.logical_inputs.front().layout == "HW",
            "BF16 tensor route MLA logical input should use the canonical packed handoff layout");
        require(
            mla_stage_fact->mla_compiled->runtime_contract.logical_inputs.front().shape.size() ==
                    2U &&
                mla_stage_fact->mla_compiled->runtime_contract.logical_inputs.front()
                        .shape.front() == 1,
            "BF16 tensor route MLA logical input should use a rank-2 packed handoff shape");
        require(
            mla_stage_fact->mla_compiled->runtime_contract.logical_inputs.front().segment_name ==
                "output_tessellated_image",
            "BF16 tensor route MLA logical input should preserve the canonical tess handoff "
            "segment");
        require(
            mla_stage_fact->mla_compiled->runtime_contract.physical_inputs.front()
        ...
        require(mla_stage_fact->mla_compiled->runtime_contract.input_bindings.front()
                ...
                "BF16 tensor route MLA input binding should preserve the canonical tess logical "
                "output index");
        require(
            mla_stage_fact->mla_compiled->runtime_contract.input_bindings.front().src_output_slot ==
                handoff->output_slot,
        ...
        for (std::size_t i = 0;
             i < detess_stage_fact->processcvu_contract->runtime_contract.input_bindings.size();
             ++i) {
          const auto& binding =
              detess_stage_fact->processcvu_contract->runtime_contract.input_bindings[i];
          const auto& input =
              detess_stage_fact->processcvu_contract->runtime_contract.logical_inputs[i];
          const auto& mla_output =
              mla_stage_fact->mla_compiled->runtime_contract.logical_outputs[i];
          ...
          require(binding.src_physical_output_index == 0,
                  "BF16 detess binding should map every logical MLA view back to the single packed "
                  "parent");
