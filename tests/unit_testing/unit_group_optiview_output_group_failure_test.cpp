...
RUN_TEST("unit_group_optiview_output_group_failure_test", ([] {
           using nlohmann::json;
           ...
           // Init failure paths.
           {
             ...
             require(!group.init(bad, 1, &err),
                     "OptiViewOutputNodeGroup should reject missing h264_caps");
             require_contains(err, "missing h264_caps",
                              "OptiViewOutputNodeGroup missing caps error mismatch");

             err.clear();
             bad.udp.h264_caps = "video/x-h264";
             require(!group.init(bad, 0, &err), "OptiViewOutputNodeGroup should reject streams=0");
           ...
           // emit_json failure: invalid stream index.
           {
             OptiViewJsonInput in;
             in.stream_idx = 7;
             OptiViewJsonResult out;
             require(!group.emit_json(in, &out), "emit_json should fail for invalid stream index");
             require_contains(out.error, "invalid stream index",
                              "emit_json invalid stream error mismatch");
           }

           // emit_json failure: missing yolo sample.
           {
             OptiViewJsonInput in;
             in.stream_idx = 0;
             OptiViewJsonResult out;
             require(!group.emit_json(in, &out), "emit_json should fail for missing yolo sample");
             require_contains(out.error, "missing yolo sample",
                              "emit_json missing sample error mismatch");
           }

           // emit_json failure: malformed bbox payload.
           {
             ...
             require(!group.emit_json(in, &out),
                     "emit_json should fail for malformed bbox payload");
             require_contains(out.error, "bbox parse failed",
                              "emit_json malformed bbox error mismatch");
           }

           // emit_json positive path.
           {
             ...
             OptiViewJsonResult out;
             require(group.emit_json(in, &out), "emit_json should succeed for valid bbox payload");
             require(out.ok, "emit_json result should report success");
             require(out.nonempty, "emit_json should report non-empty detections");

             std::string payload_json;
             require(rx.recv_one(&payload_json, 2000), "emit_json UDP payload not received");
             const json parsed = json::parse(payload_json);
             require(parsed["frame_id"].get<std::string>() == "77", "emit_json frame_id mismatch");
             require(parsed["timestamp"].get<int64_t>() == 555, "emit_json timestamp mismatch");
           }
