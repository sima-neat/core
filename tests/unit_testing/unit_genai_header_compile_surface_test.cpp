...
RUN_TEST(
    "unit_genai_header_compile_surface_test", ([] {
      ...
      message.tool_calls = Json::array({{{"id", "call_0"},
                                         {"type", "function"},
      ...
      auto genai_run =
          static_cast<GenerationResult (GenAIModel::*)(const GenerationRequest&)>(&GenAIModel::run);
      ...
      if (!speech_rejected_null) {
        throw std::runtime_error("genai::graphs::SpeechTranscriber should reject nullptr model");
      }
