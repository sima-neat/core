...
RUN_TEST("unit_boxdecode_yolo_layout_matrix_test", ([] {
           ...
           auto add_mla = [&](MpkContract& mpk, const std::string& parent_name,
             ...
             mla.output_tensors.push_back(
                 tensor(0, parent_name, dtype, {1, static_cast<std::int64_t>(size_bytes)},
                        size_bytes, MpkShapeSemantics::PackedExtent, logical_shape, {}, 0));
           ...
           auto add_unpack = [&](MpkContract& mpk, const std::string& input_name,
             ...
             unpack.input_tensors.push_back(
                 tensor(0, input_name, dtype, {1, static_cast<std::int64_t>(physical_size_bytes)},
                        physical_size_bytes, MpkShapeSemantics::PackedExtent, logical_nhwc,
                        parent_name, 0));
             unpack.output_tensors.push_back(tensor(0, output_name, dtype, physical_shape,
                                                    physical_size_bytes, output_semantics,
           ...
           for (const auto& c : cases) {
             ...
             require(extracted.has_value(),
                     std::string(c.name) + ": MPK extraction failed: " + error);
