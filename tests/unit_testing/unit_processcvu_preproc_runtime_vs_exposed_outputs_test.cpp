...
RUN_TEST("unit_processcvu_preproc_runtime_vs_exposed_outputs_test", ([] {
           ...
           const auto compiled = compile_node_contracts(std::vector<std::shared_ptr<Node>>{node},
           ...
           require(compiled.stages.front().processcvu.has_value(),
                   "preproc should compile to processcvu");
           ...
           const auto tess_runtime_it = std::find_if(
               contract.runtime_contract.logical_outputs.begin(),
               contract.runtime_contract.logical_outputs.end(), [](const auto& logical) {
