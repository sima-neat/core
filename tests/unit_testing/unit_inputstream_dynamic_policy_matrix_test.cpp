...
simaai::neat::Session make_basic_video_session(const simaai::neat::InputOptions& src_opt) {
...
RUN_TEST(
    "unit_inputstream_dynamic_policy_matrix_test", ([] {
      ...
      // guard.
      {
        ...
        Run run = session.build(std::vector<cv::Mat>{make_bgr(64, 64)}, RunMode::Async, run_opt);
      ...
      // 4) Validation parity: build-time and push-time checks use effective-max semantics.
      {
        ...
        require(has_expected_oversize_error(push_err) || has_expected_oversize_error(push_last_err),
                "parity: push path unexpected oversize validation message: e.what='" + push_err +
                    "' last_error='" + push_last_err + "'");
      }
