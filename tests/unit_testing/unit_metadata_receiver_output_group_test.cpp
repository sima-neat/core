...
RUN_TEST(
    "unit_metadata_receiver_output_group_test", ([] {
      ...
      // Configuration failure paths.
      {
        ...
        MetadataReceiverOutputNodeGroupOptions bad;
        ...
        require(!group.init(bad, 1, &err),
                "MetadataReceiverOutputNodeGroup should reject empty h264_caps");
        require_contains(err, "missing h264_caps",
                         "MetadataReceiverOutputNodeGroup h264_caps error mismatch");

        err.clear();
        bad.udp.h264_caps = "video/x-h264";
        require(!group.init(bad, 0, &err),
                "MetadataReceiverOutputNodeGroup should reject zero streams");
        require_contains(err, "streams must be > 0",
                         "MetadataReceiverOutputNodeGroup streams error mismatch");
      }
      ...
      if (!group.init(opt, 1, &init_err)) {
        if (likely_runtime_missing(init_err)) {
          throw std::runtime_error(
              "Skipping MetadataReceiverOutputNodeGroup runtime-dependent checks: " + init_err);
        }
      ...
      // Failure path: invalid stream index.
      {
        ...
        require(!group.emit_object_detection(in, &out),
                "MetadataReceiverOutputNodeGroup should reject invalid metadata stream index");
        require_contains(out.error, "invalid stream index",
                         "MetadataReceiverOutputNodeGroup invalid stream error mismatch");
      }
      ...
      // Failure path: missing yolo sample.
      {
        ...
        MetadataReceiverObjectDetectionResult out;
        require(!group.emit_object_detection(in, &out),
                "MetadataReceiverOutputNodeGroup should reject missing yolo sample");
        require_contains(out.error, "missing yolo sample",
                         "MetadataReceiverOutputNodeGroup missing yolo sample error mismatch");
      }
      ...
      // Failure path: malformed/non-bbox payload.
      {
        ...
        MetadataReceiverObjectDetectionResult out;
        require(!group.emit_object_detection(in, &out),
                "MetadataReceiverOutputNodeGroup should reject non-bbox yolo sample");
        require_contains(out.error, "bbox tensor not found",
                         "MetadataReceiverOutputNodeGroup non-bbox error mismatch");
      }
      ...
      // Failure path: malformed bbox payload with BBOX tag.
      {
        ...
        MetadataReceiverObjectDetectionResult out;
        require(!group.emit_object_detection(in, &out),
                "MetadataReceiverOutputNodeGroup should reject malformed bbox payload");
        require_contains(out.error, "bbox parse failed",
                         "MetadataReceiverOutputNodeGroup malformed bbox error mismatch");
      }

      // Happy path: valid bbox payload produces metadata datagram.
      {
        ...
        MetadataReceiverObjectDetectionResult out;
        require(group.emit_object_detection(in, &out),
                "MetadataReceiverOutputNodeGroup should emit JSON for valid bbox payload");
        require(out.ok, "MetadataReceiverOutputNodeGroup result should mark success");
        require(out.nonempty,
                "MetadataReceiverOutputNodeGroup should report non-empty detection set");
        require(out.boxes == 1, "MetadataReceiverOutputNodeGroup box count mismatch");

        std::string payload_metadata;
        require(rx.recv_one(&payload_metadata, 2000),
                "MetadataReceiverOutputNodeGroup valid emit_object_detection payload not received");

        const json parsed = json::parse(payload_metadata);
        require(parsed["frame_id"].get<std::string>() == "77",
                "MetadataReceiverOutputNodeGroup emitted frame_id mismatch");
        require(parsed["timestamp"].get<int64_t>() == 555,
                "MetadataReceiverOutputNodeGroup emitted timestamp mismatch");
        require(parsed["data"]["objects"].size() == 1,
                "MetadataReceiverOutputNodeGroup emitted object count mismatch");
      }
