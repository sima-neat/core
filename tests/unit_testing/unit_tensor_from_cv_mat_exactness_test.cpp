...
RUN_TEST("unit_tensor_from_cv_mat_exactness_test", [] {
#if !defined(SIMA_WITH_OPENCV)
  throw std::runtime_error("unit_tensor_from_cv_mat_exactness_test requires SIMA_WITH_OPENCV");
#else
  ...
  bool threw_u8 = false;
  try {
    (void)simaai::neat::tensor_from_cv_mat(
        u8, fp32_opt, "unit_tensor_from_cv_mat_exactness_test_u8");
  ...
  try {
    (void)simaai::neat::tensor_from_cv_mat(
        fp32, bf16_opt, "unit_tensor_from_cv_mat_exactness_test_bf16");
  } catch (const std::exception& e) {
    threw_bf16 = true;
