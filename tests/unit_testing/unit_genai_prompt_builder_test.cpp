...
RUN_TEST(
    "unit_genai_prompt_builder_test", ([] {
      ...
      require(empty_prompt_messages[0].content.empty(), "empty prompt content should be preserved");
      ...
      require_throws_contains(
          [&] { internal::validate_text_generation_request(direct_and_cached); }, "direct images");
      ...
      automatic_tools.tools = {{{"type", "function"}, {"function", {{"name", "get_weather"}}}}};
      ...
      GenerationRequest malformed_tools = automatic_tools;
      malformed_tools.tools = nlohmann::json::array({nlohmann::json::object()});
      require_throws_contains([&] { internal::validate_text_generation_request(malformed_tools); },
                              "tools[0]");
