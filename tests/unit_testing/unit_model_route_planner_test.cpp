...
RUN_TEST("unit_model_route_planner_test", ([] {
...

           {
             ...
             const std::string post_kind = post.nodes().front() ? post.nodes().front()->kind() : "";
             require(pre_kind == "QuantTess",
                     "strict MPK route should select QuantTess pre stage");
             require(post_kind == "DetessDequant",
...

           {
             ...
             out_shape_msg << "]";
             require(out_spec.shape == std::vector<int64_t>({1, 80, 80, 64}),
                     out_shape_msg.str());
           }
...

           {
             const auto legacy = sima_test::make_mpk_tar_fixture(
                 "model_route_legacy_missing_mpk",
                 {
                     {"etc/pipeline_sequence.json",
                      R"json({
  "pipelines": [{
  ...
  "input_buffers": [{"name": "decoder"}]
           ...
           if (std::filesystem::exists(evo_bf16_mlatess_model_path())) {
             ...
             const auto graph_chain_it = std::find_if(
                 route_plan.diagnostics.begin(), route_plan.diagnostics.end(),
             ...
             const auto graph_regions_it = std::find_if(
                 route_plan.diagnostics.begin(), route_plan.diagnostics.end(),
                 [](const std::string& diag) {
