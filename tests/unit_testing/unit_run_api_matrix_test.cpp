...
RUN_TEST(
    "unit_run_api_matrix_test", ([] {
      ...
      {
        Run closed;
        require(sima_test::throws_with([&]() { (void)closed.try_push(TensorList{seed}); },
                                       "stream is closed"),
                run_api_case("closed_try_push_tensor",
                             "Run::try_push should reject default/closed run"));
        require(
            sima_test::throws_with([&]() { (void)closed.try_push_holder({}); }, "stream is closed"),
      ...
      // Active async matrix.
      {
        ...
        require(timeout_status == PullStatus::Timeout,
                run_api_case("active_pull_timeout_status", "Run::pull timeout status mismatch"));
        require(err.code == error_codes::kRuntimePull,
        ...
        require(matched.size() == 1, run_api_case("active_pull_tensors_hit",
        ...
        require(miss.size() == 1, run_api_case("active_pull_tensors_second_hit",
                                               "pull_tensors should return one tensor again"));

        {
          const TensorList single_input = {seed};
          require(run.push(single_input),
                  run_api_case("active_push_tensor_list", "Run::push(TensorList) should succeed"));
        ...
        try {
          ...
          throw std::runtime_error(run_api_case("active_try_push_sample_copy_throw",
      ...
      // Holder-specific matrix on a fresh run.
      {
        ...
        require(
            first_samples.size() == 1,
            run_api_case("active_holder_seed_pull", "Run::pull_samples should return one sample"));
        ...
        try {
          ...
          throw std::runtime_error(run_api_case("active_try_push_holder_throw",
                                                std::string("unexpected throw: ") + e.what()));
        }
        ...
        require(holder_run.push_holder(holder),
                run_api_case("active_push_holder", "Run::push_holder should accept valid holder"));
        auto holder_roundtrip = holder_run.pull_samples(1000);
        require(
            holder_roundtrip.size() == 1 &&
                !tensors_from_sample(holder_roundtrip.front(), true).empty(),
            run_api_case("active_push_holder_output", "Run::push_holder should produce output"));

        holder_run.stop();
        require(
            !holder_run.try_push_holder(holder),
      ...
      // Sample(holder) path on its own run to avoid holder-state coupling.
      {
        ...
        try {
          ...
          throw std::runtime_error(run_api_case("active_try_push_sample_holder_throw",
                                                std::string("unexpected throw: ") + e.what()));
        }
        ...
        require(
            sima_test::throws_with([&]() { (void)holder_sample_run.try_push_holder({}); },
                                   "missing holder"),
            run_api_case("active_null_holder", "Run::try_push_holder should reject null holder"));
      ...
      // Sync contract: run() only, no push/pull.
      {
        ...
        require(
            sima_test::throws_with([&]() { (void)sync_run.try_push(TensorList{seed}); },
                                   "not allowed in sync mode"),
            run_api_case("sync_try_push_tensor_throws", "Run::try_push should throw in sync mode"));
        require(sima_test::throws_with([&]() { (void)sync_run.pull_tensors(10); },
                                       "not allowed in sync mode"),
                run_api_case("sync_pull_tensors_throws",
                             "Run::pull_tensors should throw in sync mode"));
        require(sima_test::throws_with([&]() { (void)sync_run.pull_samples(10); },
                                       "not allowed in sync mode"),
      ...
      // Format/depth mismatch path.
      {
        ...
        require(!msg.empty(), run_api_case("format_depth_mismatch_error_msg",
