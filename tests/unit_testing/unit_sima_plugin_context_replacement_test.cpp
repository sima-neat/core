...
RUN_TEST("unit_sima_plugin_context_replacement_test", ([] {
           ...
           const auto* new_stage = sima_plugin_manifest_context_stage_lookup_typed_checked(
               pipeline_context, "stage_new_id", "stage_new",
           ...
           require(gst_bin_add(GST_BIN(pipeline), late),
                   "late child should be addable to pipeline");
           ...
           const auto* late_stage = sima_plugin_manifest_context_stage_lookup_typed_checked(
               late_context, "stage_new_id", "stage_new",
               SIMA_PLUGIN_STAGE_PAYLOAD_BOXDECODE, &status);
