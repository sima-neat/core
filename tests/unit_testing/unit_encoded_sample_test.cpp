...
int main() {
  ...
  require(sample.tensor.has_value(), "encoded Sample missing tensor");
  require(sample.tensor->semantic.encoded.has_value(), "encoded Sample missing semantic");
  require(sample.tensor->semantic.encoded->codec == simaai::neat::EncodedSpec::Codec::H264,
          "encoded Sample codec mismatch");
  require(sample.tensor->dtype == TensorDType::UInt8, "encoded Sample dtype mismatch");
  require(sample.tensor->shape.size() == 1 && sample.tensor->shape[0] == 10,
  ...
  RunOptions run_opt;
  Run run = p.build(sample, RunMode::Async, run_opt);

  require(run.push(sample), "encoded Sample push failed");
  Sample sample2 = make_encoded_sample(std::vector<uint8_t>(7, 0xCD), "video/x-h264");
  require(run.push(sample2), "encoded Sample push2 failed");
  ...
  require(run.push(bad), "encoded Sample push3 failed");
