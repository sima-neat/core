...
nlohmann::json collect_tool_calls(ToolCallFormat format, const std::vector<std::string>& chunks) {
  ToolCallStreamParser parser(format);
...
RUN_TEST(
    "unit_genai_tool_call_parser_test", ([] {
      ...
      require_content_chunks(collect_content({"T", "h", "e", "re", " are", " facts."}),
                             {"T", "h", "e", "re", " are", " facts."});
      require_content_chunks(collect_content({"<b", "old>", " text"}), {"<b", "old>", " text"});
      require_content_chunks(collect_content({"callout", ": this is just prose"}),
                             {"callout", ": this is just prose"});

      require_first_call(collect_tool_calls(ToolCallFormat::Gemma, {"ca", "ll:get_weather{city:",
                                                                    "<|\"|>To", "kyo<|\"|>}"}),
                         "get_weather", "Tokyo");
      require_first_call(
          collect_tool_calls(
      ...
      require_first_call(collect_tool_calls(ToolCallFormat::Lfm,
                                            {"<|tool_call_start|>[get_weather(city=\"Tokyo\")]",
                                             "<|tool_call_end|>"}),
                         "get_weather", "Tokyo");
      require_first_call(collect_tool_calls(ToolCallFormat::GenericJson,
                                            {"[{\"name\":\"get_",
                                             "weather\",\"arguments\":{\"city\":\"Tokyo\"}}]"}),
                         "get_weather", "Tokyo");
      require_first_call(
          collect_tool_calls(ToolCallFormat::Qwen, {"<tool_",
                                                    "call>\n{\"name\":\"get_weather\","
      ...
      require_call_names(
          ...
          {"first", "second"});
      require_call_names(
          collect_tool_calls(ToolCallFormat::Gemma, {"call:first{}", " call:second{}"}),
          {"first", "second"});

      require_first_call(
          collect_tool_calls(ToolCallFormat::Mistral, {"[TOOL_CALLS]\n[{\"name\":\"get_weather\",",
                                                       "\"arguments\":{\"city\":\"Tokyo\"}}]"}),
          "get_weather", "Tokyo");

      require(simaai::llima::tool_call_format_for_model("vlm-lfm2_vl") == ToolCallFormat::Lfm,
              "LFM format lookup mismatch");
      require(simaai::llima::tool_call_special_tokens(ToolCallFormat::Lfm) ==
                  std::vector<std::string>{"<|tool_call_start|>", "<|tool_call_end|>"},
      ...
      require(simaai::llima::tool_call_format_for_model("llm-qwen3") == ToolCallFormat::Qwen,
      ...
      const auto quoted_brace = collect_tool_calls(
          ToolCallFormat::Llama, {"{\"name\":\"search\",\"parameters\":{\"query\":\"a}b\\\\c\"}}"});
      ...
      require_content_chunks(collect_content({"{\"name\":\"Alice\",\"age\":30}"}),
                             {"{\"name\":\"Alice\",\"age\":30}"});
      require_content_chunks(collect_content({"{\"name\":\"weather\",\"arguments\":\"not-json\"}"}),
                             {"{\"name\":\"weather\",\"arguments\":\"not-json\"}"});
      require_content_chunks(collect_content({"[]"}), {"[]"});
      require_content_chunks(
          collect_content({"<tool_call>{\"name\":\"weather\",\"arguments\":{}}</tool_call> more"}),
      ...
      const std::string weather_call = "{\"name\":\"weather\",\"arguments\":{\"city\":\"Tokyo\"}}";
      require(simaai::llima::try_parse_tool_calls(ToolCallFormat::Llama, weather_call, {"search"})
                  .is_null(),
              "undeclared tool name should be rejected");
      require(!simaai::llima::try_parse_tool_calls(ToolCallFormat::Llama, weather_call, {"weather"})
                   .is_null(),
