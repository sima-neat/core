...
sima::MlaStaticContract make_projection(const sc::ModelExecutionPlan& plan) {
  ...
  for (const auto& port : plan.backend_ports()) {
    ...
    // The extracted MPK stage starts with the logical tensor extent. Strict
    // projection verifies that evidence before replacing it with the ELF's
    // exact physical port extent.
...
sc::ModelExecutionPlan make_padded_qmla_output_plan() {
  sc::ModelExecutionPlanData data;
  data.contract_version = "2.0.0";
  data.values = {
      sc::ValueSpec{0U, "input", 16U, "float32", sc::TensorShape{1, 4}, "normal",
                    {}, sc::ValueRepresentation::Dense},
      sc::ValueSpec{1U, "logits", 109200U, "float32", sc::TensorShape{1, 300, 91},
                    "normal", {}, sc::ValueRepresentation::Dense,
                    sc::ReadExpression{1U, 0U, {110400, 368, 4}}},
  };
  data.model_inputs = {0U};
  sc::OpSpec mla;
  mla.id = 0U;
  ...
  data.ops.push_back(std::move(mla));
  data.backend_ports = {
      sc::BackendPortSpec{0U, sc::BackendPortDirection::Input, 0U,
                          "data.ifm.persistent.afe_direct_input_0.b0", 0U, 16U,
                          sc::kLegacyEvoCmaRegionAlignmentBytes,
                          sc::BackendPortAlignmentAuthority::LegacyPolicy,
                          sc::BackendPortAccess::ReadOnly},
      sc::BackendPortSpec{0U, sc::BackendPortDirection::Output, 0U,
                          "data.ofm.persistent.afe_mla_output_0.b0", 1U, 110400U,
                          sc::kLegacyEvoCmaRegionAlignmentBytes,
                          sc::BackendPortAlignmentAuthority::LegacyPolicy,
                          sc::BackendPortAccess::WriteOnly},
  };
  data.model_outputs = {{0U, "logits", 1U}};
  std::string error;
  auto plan = sc::ModelExecutionPlan::create(std::move(data), &error);
  require(plan.has_value(), "padded QMLA execution plan must be valid: " + error);
  return std::move(*plan);
}
    ...
    "unit_dmabuf_plan_contract_projection_test", ([] {
      ...
      require(contract.logical_outputs[7].logical_index == 7 &&
                  ...
                  contract.logical_outputs[7].size_bytes ==
                      plan.backend_ports()[2U + 7U].physical_extent_bytes,
      ...
      const auto padded_plan = make_padded_qmla_output_plan();
      auto padded_contract = make_projection(padded_plan);
      auto padded_sources =
          sc::resolve_mla_input_physical_sources(padded_plan, {}, &error);
      require(padded_sources.has_value(),
              "padded QMLA input carrier must resolve: " + error);
      require(sc::apply_dmabuf_plan_contract_projection(
                  padded_plan, &padded_contract, *padded_sources, &error),
              "padded QMLA contract must project: " + error);
      require(padded_contract.dispatcher_physical_outputs.size() == 1U &&
                  padded_contract.dispatcher_physical_outputs[0].size_bytes == 110400U &&
                  padded_contract.physical_outputs.size() == 1U &&
                  padded_contract.physical_outputs[0].size_bytes == 110400U,
              "ELF physical extent owns both dispatcher and allocated MLA carriers");
      require(padded_contract.logical_outputs.size() == 1U &&
                  padded_contract.logical_outputs[0].size_bytes == 109200U &&
                  padded_contract.logical_outputs[0].shape ==
                      std::vector<std::int64_t>({1, 300, 91}) &&
                  padded_contract.logical_outputs[0].stride_bytes ==
                      std::vector<std::int64_t>({110400, 368, 4}),
              "public logits retain logical extent and exact QMLA row pitch");
      require(padded_contract.frame_arena_size_bytes >= 110400U,
              "frame arena accounts conservatively for the full padded carrier");
