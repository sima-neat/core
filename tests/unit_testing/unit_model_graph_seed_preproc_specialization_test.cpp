...
simaai::neat::Tensor make_image_tensor(int width, int height,
  ...
  tensor.layout = channels == 1 ? simaai::neat::TensorLayout::HW
                                : simaai::neat::TensorLayout::HWC;
...
const simaai::neat::PreprocOptions& require_model_preproc_options(
    const std::vector<std::shared_ptr<simaai::neat::Node>>& nodes, const std::string& where) {
...
const simaai::neat::PreprocOptions& require_single_segment_preproc_options(
    const simaai::neat::runtime::ExecutionGraphPlan& plan, const std::string& where) {
  require(plan.pipeline_segments.size() == 1U,
          where + ": expected one linear pipeline segment, got " +
...
RUN_TEST("unit_model_graph_seed_preproc_specialization_test", ([] {
           ...
           const Sample bgr_seed = make_image_tensor_sample(
               321, 123, ImageSpec::PixelFormat::BGR);
           const auto expected_bgr_nodes = internal::ModelAccess::build_public_route_nodes_for_seed(
               model, route_opt, bgr_seed);
           const auto default_nodes =
               internal::ModelAccess::build_public_route_nodes(model, route_opt);
           ...
           try {
             (void)runtime::compile_public_graph(missing_image_options_graph, RunOptions{},
                                                 bgr_seed);
             throw std::runtime_error(
                 "expected raw image seed without model-managed Preproc to fail");
           ...
           require_same_preproc_seed_contract(
               expected_bgr, graph_bgr, "Model::build(Sample) vs model.graph().build(Sample)");

           const Sample rgb_seed = make_image_tensor_sample(
               257, 199, ImageSpec::PixelFormat::RGB);
           const auto expected_rgb_nodes = internal::ModelAccess::build_public_route_nodes_for_seed(
               model, route_opt, rgb_seed);
           ...
           const auto expected_cv_nodes = internal::ModelAccess::build_public_route_nodes_for_seed(
               model, route_opt, cv_seed);
