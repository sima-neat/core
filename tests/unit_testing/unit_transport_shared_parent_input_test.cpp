...
CompiledProcessCvuContract make_shared_parent_detess_contract() {
  ...
  static const std::array<std::vector<std::int64_t>, 6> kShapes = {
      std::vector<std::int64_t>{80, 80, 64}, std::vector<std::int64_t>{40, 40, 64},
      std::vector<std::int64_t>{20, 20, 64}, std::vector<std::int64_t>{80, 80, 80},
      std::vector<std::int64_t>{40, 40, 80}, std::vector<std::int64_t>{20, 20, 80},
  };
  static const std::array<std::vector<std::int64_t>, 6> kStrides = {
      std::vector<std::int64_t>{10240, 128, 2}, std::vector<std::int64_t>{5120, 128, 2},
      std::vector<std::int64_t>{2560, 128, 2},  std::vector<std::int64_t>{12800, 160, 2},
      std::vector<std::int64_t>{6400, 160, 2},  std::vector<std::int64_t>{3200, 160, 2},
  };
  ...
  for (std::size_t i = 0; i < kShapes.size(); ++i) {
    ...
    auto logical = build_logical_input_static_spec(
        static_cast<int>(i), static_cast<int>(i), static_cast<int>(i), kShapes[i], "BF16", "HWC",
        logical_name, logical_name, local_segment_name, kByteOffsets[i], kLogicalSizes[i]);
    ...
    compiled.runtime_contract.physical_inputs.push_back(
        build_physical_buffer_static_spec(static_cast<int>(i), static_cast<int>(i),
                                          kLogicalSizes[i], DeviceKind::Evxx, local_segment_name));
  }

  compiled.runtime_contract.logical_outputs.push_back(build_logical_output_static_spec(
      0, 0, 0, 0, 0, {80, 80, 64}, "BF16", "HWC", "head0", "head0", "output_tensor", 0, 819200U));
  compiled.runtime_contract.physical_outputs.push_back(
      build_physical_buffer_static_spec(0, 0, 819200U, DeviceKind::Evxx, "output_tensor"));
...
void verify_shared_parent_input_transport_normalization() {
  ...
  const auto runtime = pipeline_internal::sima::stagesemantics::
