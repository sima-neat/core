...
RUN_TEST("unit_post_region_materialization_test", ([] {
           using namespace simaai::neat;
           using namespace simaai::neat::internal;

           const auto check_model = [](const std::filesystem::path& model_path,
                                       const std::vector<std::pair<RouteRegionKind,
             ...
             const bool expects_post_cast = std::find(expected_post_chain.begin(),
                                                      expected_post_chain.end(),
                                                      SessionPostStageOp::Cast) !=
                                            expected_post_chain.end();
           ...
           check_model(
               ...
               {6U},
               {SessionPostStageOp::DetessDequant},
               {"DetessDequant"});
           ...
           check_model(
               int8_mlatess_model_path(),
               {
                   {RouteRegionKind::FanoutMap,
                    pipeline_internal::sima::RouteGraphKernelKind::Dequantize},
               },
               {6U},
               {SessionPostStageOp::Dequantize},
               {"Dequant"});

           const auto check_packed_processcvu_payload =
               [](const std::filesystem::path& model_path,
                 ...
                 require(mpk_opt.has_value(),
                         "packed routed-input regression requires MPK contract for " +
                             model_path.filename().string());
                 ...
                 require(diagnostics.errors.empty(),
                         "post group should compile without diagnostics for " +
                             model_path.filename().string());
                 ...
                 require(payload.graph_family == expected_graph_family,
                         "unexpected packed post graph family for " +
                             model_path.filename().string());
           ...
           check_packed_processcvu_payload(
               int8_mlatess_model_path(), "dequantize", 6);
           ...
           if (std::filesystem::exists(bf16_int8_model_path())) {
             ...
             const auto compiled =
                 compile_node_contracts(post.nodes(), ContractCompileInput{}, &diagnostics);
             require(diagnostics.errors.empty(),
                     "BF16 post group should compile without diagnostics");
             require(!compiled.stages.empty(),
                     "BF16 post group should materialize at least one compiled stage");
             const auto* runtime =
                 compiled_runtime_contract_from_stage(&compiled.stages.back());
             require(runtime != nullptr,
                     "BF16 post group should expose a final runtime contract");
               ...
               [](const auto& runtime,
                 ...
                 for (std::size_t i = 0; i < 6U; ++i) {
                   ...
                   require(!expected_segment.empty(),
                           label + " requires an MPK-published source segment for " +
                               model_name);
                   if (std::find(expected_physical_segments.begin(),
                                 expected_physical_segments.end(),
                 ...
                 require(runtime.physical_inputs.size() == expected_physical_segments.size(),
                         label + " should preserve MPK physical input cardinality for " +
                             model_name);
                 for (std::size_t i = 0; i < expected_physical_segments.size(); ++i) {
                   require(runtime.physical_inputs[i].segment_name ==
                               expected_physical_segments[i],
                 ...
                 for (std::size_t i = 0; i < 6U; ++i) {
                   ...
                   const auto physical_it =
                       std::find(expected_physical_segments.begin(),
                                 expected_physical_segments.end(),
                                 expected_segment);
                   require(physical_it != expected_physical_segments.end(),
                           label + " should have a matching local physical input for " +
                               model_name);
                   const int expected_local_physical_index =
                       static_cast<int>(std::distance(expected_physical_segments.begin(),
                                                      physical_it));
                   ...
                   require(logical.byte_offset == expected_byte_offset,
                           label + " logical input should preserve MPK byte offset for " +
                               model_name);
                   require(binding.source_segment_name == expected_segment,
                           label + " binding should preserve MPK source segment for " +
                               model_name);
                   ...
                   require(binding.src_physical_byte_offset == expected_byte_offset,
                           label + " binding should preserve routed MPK byte offset for " +
                               model_name);
                 }
               };

           const auto check_routed_input_addressing =
               [&](const std::filesystem::path& model_path,
                  const std::string& expected_graph_family) {
                 if (!std::filesystem::exists(model_path)) {
                   return;
           ...
           check_routed_input_addressing(
               int8_mlatess_model_path(), "dequantize");
           if (std::filesystem::exists(int8_model_path())) {
             Model::Options opt;
             opt.preprocess.kind = InputKind::Tensor;
             opt.preprocess.enable = AutoFlag::On;
             ...
             require(detess_stage.payload.graph_family == "detessdequant",
           ...
           const auto check_model_managed_dequant_routed_input_addressing =
               [&](const std::filesystem::path& model_path) {
                 if (!std::filesystem::exists(model_path)) {
                   return;
                 }
                 ...
                 const auto& mpk_opt = pack.mpk_contract();
                 require(mpk_opt.has_value(),
                         "model-managed dequant regression requires MPK contract for " +
                             model_path.filename().string());
                 const auto published_inputs =
                     pipeline_internal::sima::get_mla_published_outputs_contract(*mpk_opt);
                 require(published_inputs.size() == 6U,
                 ...
                 require(dequant_stage_fact->processcvu_contract->payload.graph_family == "dequantize",
                         "model-managed dequant regression expects dequantize processcvu family for " +
                             model_path.filename().string());
                 check_routed_input_addressing_matches_published(
                     dequant_stage_fact->processcvu_contract->runtime_contract,
                     published_inputs,
                 ...
                 const auto compiled =
                     compile_node_contracts(post.nodes(), ContractCompileInput{}, &diagnostics);
                 require(diagnostics.errors.empty(),
                         "model-managed dequant regression expects post group to compile without diagnostics for " +
                             model_path.filename().string());
                 require(compiled.stages.size() == 1U,
                 ...
                 require(compiled.stages.front().processcvu.has_value(),
                         "model-managed dequant regression expects a processcvu dequant compiled stage for " +
                             model_path.filename().string());
                 require(compiled.stages.front().processcvu->payload.graph_family == "dequantize",
                         "model-managed dequant regression expects compiled dequantize processcvu family for " +
                             model_path.filename().string());

                 check_routed_input_addressing_matches_published(
                     compiled.stages.front().processcvu->runtime_contract,
                     published_inputs,
                     "compiled stage", model_path.filename().string());
               };

           check_model_managed_dequant_routed_input_addressing(
               int8_mlatess_model_path());
           ...
           const auto check_preadapter_transport_vs_published_names =
               [](const std::filesystem::path& model_path,
                 ...
                 require(payload.graph_family == expected_graph_family,
                         "unexpected preadapter graph family for " +
                             model_path.filename().string());
                 require(payload.default_output_names.size() == 1U,
           ...
           check_preadapter_transport_vs_published_names(
               int8_mlatess_model_path(), "quantize", "output_tensor");
         }));
