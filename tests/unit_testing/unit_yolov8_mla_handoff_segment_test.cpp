...
RUN_TEST("unit_yolov8_mla_handoff_segment_test", ([] {
           ...
           require(!infer.nodes().empty(),
                   "YOLOv8 inference fragment should compile from the local MPK");
           ...
           require(!compiled.stages.empty(),
                   "YOLOv8 inference fragment should emit a container stage");

           const CompiledNodeContract* mla_stage = nullptr;
           const auto visit_stage = [&](const auto& self,
