...
RUN_TEST("unit_modelpack_extract_test", ([] {
           namespace fs = std::filesystem;

           const std::string strict_tar =
               sima_test::resolve_modelzoo_tar("yolo_v9c_seg", sima_test::repo_root_for_modelzoo());
           require(!strict_tar.empty(),
                   "missing strict fixture tar; run sima-cli modelzoo get yolo_v9c_seg");
           ...
           const fs::path stale_keep = fs::path(extract_root) / "proc_99999992";
           {
             ...
             dead_out << "orphan\n";
             std::ofstream keep_out(stale_keep / ".sima_modelpack_keep",
           ...
           require(!fs::exists(stale_dead),
                   "stale dead proc_* extraction root should be auto-removed");
...

           {
             ...
             const fs::path keep_proc_root =
                 fs::path(keep_pack.etc_dir()).parent_path().parent_path();
           ...
           require(throws_with(
                       [&]() {
                         const auto legacy =
                             sima_test::make_mpk_tar_fixture("modelpack_extract_legacy_missing_mpk",
                                                             {
