...
Tensor make_route_tensor(int logical_index, int physical_index, int route_slot, int memory_index,
  ...
  tensor.strides_bytes = {static_cast<std::int64_t>(sizeof(float)), static_cast<std::int64_t>(sizeof(float))};
...
RUN_TEST("unit_model_input_spec_contract_test", ([] {
  ...
  "input_buffers": [{"name": "decoder"}]
...

           {
             ...
             require(singular_spec_threw,
                     "singular input_spec must reject multi-ingress models");
...

           {
             ...
             require(localized.shape == original_shape,
                     "branch-local remap must preserve tensor shape");
           ...
           {
             Tensor first =
                 make_route_tensor(0, 0, 0, 0, 64U, 0x11, "image_l", "cast_0", "cast_0");
             Tensor second =
                 make_route_tensor(1, 0, 1, 0, 32U, 0x22, "image_uv", "cast_1", "cast_1");
             ...
             require(joined.size() == 2U,
                     "joined-ingress materialization must preserve tensor count");
             ...
             require(joined[0].byte_offset == 0,
                     "first joined tensor must start at byte offset zero");
           ...
           {
             Tensor first =
                 make_route_tensor(0, 0, 0, 0, 64U, 0x33, "image_l", "cast_0", "cast_0");
             Tensor second =
                 make_route_tensor(0, 0, 0, 0, 32U, 0x44, "image_uv", "cast_1", "cast_1");
             const std::size_t first_bytes = first.dense_bytes_tight();
             const std::vector<internal::IngressConsumerTensorIdentity> consumer_identities = {
                 {.logical_index = 0, .physical_index = 0, .route_slot = 0, .memory_index = 0},
             ...
             require(joined.size() == 2U,
                     "joined-ingress materialization must preserve tensor count for shared-physical contracts");
             require(joined[0].storage != nullptr && joined[0].storage == joined[1].storage,
                     "joined-ingress materialization must still compact shared-physical inputs into one storage");
             require(joined[0].route.logical_index == 0 && joined[0].route.physical_index == 0 &&
                         joined[0].route.route_slot == 0 && joined[0].route.memory_index == 0,
