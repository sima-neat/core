...
RUN_TEST("unit_preprocess_roi_batch_pack_test", ([] {
  ...
  require(packed.shape == std::vector<int64_t>({2, 2, 3, 3}), "packed tensor shape mismatch");
  ...
  require(packed.semantic.preprocess.has_value(), "packed tensor missing preprocess metadata");
  ...
  for (std::size_t n = 0; n < refs.size(); ++n) {
    for (int y = 0; y < refs[n].rows; ++y) {
      ...
      for (std::size_t b = 0; b < row_bytes; ++b) {
        ...
        require(packed == src[b],
                "packed tensor byte layout mismatch");
