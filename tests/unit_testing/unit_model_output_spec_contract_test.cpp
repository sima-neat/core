...
RUN_TEST("unit_model_output_spec_contract_test", ([] {
...

           {
             ...
             const std::vector<std::vector<int64_t>> expected_shapes = {
                 {1, 80, 80, 64},  {1, 40, 40, 64}, {1, 20, 20, 64},  {1, 80, 80, 80},
                 {1, 40, 40, 80},  {1, 20, 20, 80}, {1, 80, 80, 32},  {1, 40, 40, 32},
                 {1, 20, 20, 32},  {1, 160, 160, 32},
             };
             ...
             for (size_t i = 0; i < specs.size(); ++i) {
               ...
               require(spec.rank == static_cast<int>(expected_shapes[i].size()),
                       "Model::output_specs rank mismatch at index " + std::to_string(i) +
                           ", got " + std::to_string(spec.rank));
               require(spec.shape == expected_shapes[i],
                       "Model::output_specs shape mismatch at index " + std::to_string(i) +
                           ", got " + shape_to_string(spec.shape));
             }
