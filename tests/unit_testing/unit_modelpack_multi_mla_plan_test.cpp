...
using sima_test::write_topology_elf;
...
void write_host_module(const std::filesystem::path& path, const std::uint64_t rows,
  ...
  for (std::size_t index = 0U; index < input_names.size(); ++index) {
    nodes.push_back(
        {{"op", "null"}, {"name", input_names[index]}, {"inputs", nlohmann::json::array()}});
  ...
  const nlohmann::ordered_json graph_json{
      ...
      {"attrs",
       {{"shape", nlohmann::json::array({"list_shape", shapes})},
...
void write_boxdecode_cast_tail_manifest(const std::filesystem::path& path,
  ...
  const std::string model_input_scalar = mla_scalar == "bfloat16" ? "float32" : "bfloat16";
  const auto model_input_bytes = input_elements * scalar_bytes(model_input_scalar);
...
std::vector<nlohmann::json> direct_tvm_contracts(const std::string& fragment) {
  ...
  for (std::size_t cursor = 0U;;) {
    ...
    require(decoded != nullptr && decoded_size != 0U, "failed to decode direct TVM lane contract");
    result.push_back(nlohmann::json::parse(reinterpret_cast<const char*>(decoded),
...
RUN_TEST(
    "unit_modelpack_multi_mla_plan_test", ([] {
      ...
      require(model.execution_admission().eligible(),
      ...
      require(first_mla && first_mla->name == "MLA_encoder" && last_mla &&
                  ...
                  simaai::neat::pipeline_internal::sima::get_mla_stage_io_contract(mpk) == nullptr,
      ...
      require(first.frame_arena_role == FrameArenaRole::Allocate &&
                  ...
                  second.frame_arena_escape_policy == sc::ArenaEscapePolicy::CpuMappablePublic,
              "only the terminal public MLA must own a separate CPU-visible output pool");
      require(first.frame_arena_size_bytes != 0U && second.frame_arena_size_bytes != 0U,
      ...
      const auto make_mla_a65_package = [&](const std::string& label, const std::uint64_t rows) {
      ...
      require(singleton_model.infer_output_name() == "APU_1",
              "default inference naming must retain the terminal A65 stage before admission");
      const auto singleton_capability = simaai::neat::internal::extract_route_capability(
          singleton_model, simaai::neat::internal::PreprocessPlannerResult{});
      require(singleton_capability.ordered_post_ops.empty() &&
                  !singleton_capability.has_external_post &&
                  singleton_capability.egress_contracts.size() == 1U &&
                  singleton_capability.egress_contracts.front().source_stage == "APU_1",
              "terminal A65 must retain its output contract without becoming a CVU adapter");
      simaai::neat::Model::Options singleton_options;
      singleton_options.preprocess.kind = simaai::neat::InputKind::Tensor;
      singleton_options.preprocess.enable = simaai::neat::AutoFlag::Off;
      const simaai::neat::Model singleton_public_model(singleton_package.string(),
                                                       singleton_options);
      const auto singleton_output_specs = singleton_public_model.output_specs();
      require(singleton_public_model.infer_output_name() == "APU_1" &&
                  singleton_output_specs.size() == 1U &&
                  singleton_output_specs.front().shape == std::vector<int64_t>({1, 1225}) &&
                  singleton_output_specs.front().dtypes ==
                      std::vector<simaai::neat::TensorDType>{simaai::neat::TensorDType::Float32},
              "default inference metadata must describe its terminal A65 tensor");
      singleton_model.prepare_for_execution();
      require(singleton_model.infer_output_name() ==
                  singleton_model.execution_plan().infer.back().stage_name,
              "descriptive inference naming must match the executable terminal stage");
      const auto singleton_facts = singleton_model.stage_facts_for_model_stage(ModelStage::MlaOnly);
      require(
          singleton_facts.size() == 2U && singleton_facts[0].mla_compiled.has_value() &&
              singleton_facts[1].transport_compiled.has_value() &&
              !singleton_facts[0].mla_compiled->runtime_contract.physical_outputs.empty() &&
              !singleton_facts[1].transport_compiled->runtime_contract.logical_inputs.empty() &&
              singleton_facts[0].mla_compiled->runtime_contract.physical_outputs[0].size_bytes ==
                  4912U &&
              singleton_facts[1].transport_compiled->runtime_contract.logical_inputs[0].shape ==
                  std::vector<std::int64_t>({1, 1225}),
      ...
      const auto make_relation_tail_package = [&](const std::string& label, const bool interleaved,
                                                  const bool add_unused_relation = false) {
        const fs::path package = root.parent_path() / label;
        fs::remove_all(package, ec);
        fs::create_directories(package / "etc", ec);
      ...
      const auto select_relation_tail_route = [&](const fs::path& package) {
        ...
        ModelPack candidate(package.string(), "application/vnd.simaai.tensor", "FP32", 1, 4, 1, 1,
                            false, {}, {}, {}, PipelineType::Preproc, "decoder", 4, 4, 0, -1, {},
                            {}, policy,
                            /*cleanup_extracted_model_data=*/false);
        const auto full_proof = candidate.execution_admission().proof;
        const auto full_digest = candidate.execution_plan_digest();
      ...
      const auto& interleaved_admission = interleaved_model.execution_admission();
      require(interleaved_model.execution_plan_digest() == interleaved_full_digest,
              "terminal selection changed the immutable full-plan digest");
      require_proof_unchanged(interleaved_full_proof, interleaved_admission.proof);
      const auto interleaved_plan = interleaved_model.execution_plan();
      require(
          std::any_of(interleaved_plan.post.begin(), interleaved_plan.post.end(),
                      [](const auto& stage) { return stage.kind == ExecutionStageKind::Dequant; }),
      ...
      const std::array<std::string, 6U> expected_interleaved_names{
          "bbox_raw_0", "bbox_raw_1", "bbox_raw_2", "class_raw_0", "class_raw_1", "class_raw_2"};
      ...
      require(prefix_model.execution_plan_digest() == prefix_full_digest,
              "dense-prefix terminal selection changed the full-plan digest");
      require_proof_unchanged(prefix_full_proof, prefix_model.execution_admission().proof);

      // Optional exact-artifact gate for the App48 YOLO26 package whose
      // compiler order interleaves Slice and Dequant operations.
      if (const char* yolo26_package = std::getenv("SIMANEAT_EXACT_YOLO26_INTERLEAVED_PACKAGE");
          yolo26_package != nullptr && *yolo26_package != '\0') {
        simaai::neat::internal::InferenceTerminalPolicy policy;
        policy.mla_only = true;
        ModelPack yolo26_model(yolo26_package, "application/vnd.simaai.tensor", "FP32", 3, 640, 640,
                               3, false, {}, {}, {}, PipelineType::QuantTess, "decoder", 4, 4, 0,
                               -1, {}, {}, policy,
                               /*cleanup_extracted_model_data=*/false);
        const auto full_proof = yolo26_model.execution_admission().proof;
        const auto full_digest = yolo26_model.execution_plan_digest();
        simaai::neat::pipeline_internal::sima::ModelManagedRouteFlags route_flags;
        route_flags.boxdecode_selected = true;
        route_flags.terminal_consumer_owns_tensor_tail = true;
        yolo26_model.set_model_managed_stage_facts(route_flags, {ExecutionStageKind::BoxDecode});
        require(yolo26_model.execution_plan_digest() == full_digest,
                "exact YOLO26 terminal selection changed the full-plan digest");
        require_proof_unchanged(full_proof, yolo26_model.execution_admission().proof);
      ...
      try {
        ...
        multirow_rejected =
            std::string(failure.what()).find("A65 input has no exact typed storage binding") !=
            std::string::npos;
      }
      ...
      write_topology_elf(join_package / "share" / "first.elf",
                         "data.ifm.persistent.afe_direct_input_0.b0", 16U,
                         "data.ofm.persistent.afe_mla_output_0.b0", 4912U);
      write_topology_elf(join_package / "share" / "last.elf",
                         "data.ifm.persistent.afe_direct_input_0.b0", 2450U,
                         "data.ofm.persistent.afe_mla_output_0.b0", 2464U);
      write_host_module(join_package / "lib" / "host_one.so", 1U, {"root_input"});
      write_host_module(join_package / "lib" / "host_join.so", 1U, {"root_input", "chain_input"});
      ModelPack join_model(join_package.string());
      join_model.prepare_for_execution();
      const auto join_plan = join_model.execution_plan();
      require(join_plan.infer.size() == 5U && join_plan.infer[0].kind == ExecutionStageKind::Mla &&
      ...
      const auto join_capability = simaai::neat::internal::extract_route_capability(
          join_model, simaai::neat::internal::PreprocessPlannerResult{});
      const auto join_semantics = simaai::neat::internal::build_model_semantics(join_model);
      const auto join_route = simaai::neat::internal::build_route_plan(
          simaai::neat::Model::Options{}, join_semantics, &join_capability, &join_model);
      require(join_capability.ordered_post_ops.empty() &&
                  std::none_of(join_route.diagnostics.begin(), join_route.diagnostics.end(),
                               [](const auto& diagnostic) {
                                 return diagnostic.starts_with(
                                     "session-route: graph_post_chain_raw=");
                               }),
              "interstitial CVU work must not reappear as terminal graph postprocessing");
      const auto join_contracts =
          direct_tvm_contracts(join_model.backend_fragment(ModelStage::MlaOnly));
      require(join_contracts.size() == 2U && join_contracts[0].at("storage_domain") == "cma" &&
                  join_contracts[1].at("storage_domain") == "cma" &&
                  join_contracts[0].at("allocation_provenance") == "core_allocated" &&
                  join_contracts[1].at("allocation_provenance") == "core_allocated" &&
      ...
      constexpr std::string_view relative_path_json =
          ...
          "\"keep\" : [ 2, 1 ] }\n";
      const fs::path archive_source = root.parent_path() / "neat-modelpack-json-preservation-unit";
      ...
      write_archive_manifest(archive_source / "etc" / "modelpack_json_preservation_mpk.json");
      ...
      const std::string source_mpk =
          read_text(archive_source / "etc" / "modelpack_json_preservation_mpk.json");
      ...
      const std::string archive_command = "tar -czf " + shell_quote(archive.string()) + " -C " +
              ...
              "failed to create synthetic ModelPack archive");
      {
        ...
        const std::string extracted_mpk =
            read_text(extracted_etc / "modelpack_json_preservation_mpk.json");
      ...
      if (const char* real_package = std::getenv("SIMANEAT_AFE21_MULTI_MLA_PACKAGE");
        ...
        ModelPack real_model(real_package, "application/vnd.simaai.tensor", "FP32", /*depth=*/1,
                             /*max_width=*/640, /*max_height=*/480, /*max_depth=*/1,
                             /*normalize=*/false, {}, {}, /*preproc_next_cpu=*/{},
                             PipelineType::QuantTess);
        require(real_model.execution_admission().eligible(),
                "exact AFE 2.1 multi-MLA/A65 package failed ModelPack admission");
        const auto real_plan = real_model.execution_plan();
        require(real_plan.pre.empty() && real_plan.post.empty() && !real_plan.infer.empty(),
        ...
        for (std::size_t index = 0; index < real_plan.infer.size(); ++index) {
          ...
          case ExecutionStageKind::Cast:
            ...
            break;
          default:
            require(false, "exact RF-DETR schedule contains work outside MLA, direct A65, or "
                           "registered CVU Cast commands");
          }
        }
        require(mla_indices.size() == 2U && host_indices.size() == 3U && !cast_indices.empty() &&
                    ...
                    mla_indices[0] < host_indices[0] && host_indices[0] < host_indices[1] &&
                    host_indices[1] < host_indices[2] && host_indices[2] < mla_indices[1],
                "exact RF-DETR schedule lost its two MLA boundaries or three ordered "
                "direct A65 commands");

        real_model.set_model_managed_stage_facts(std::nullopt, {});
        const auto real_facts = real_model.stage_facts_for_model_stage(ModelStage::MlaOnly);
        ...
        const auto& initial_mla = real_facts[mla_indices[0]].mla_compiled->runtime_contract;
        const auto align16 = [](const std::uint64_t value) {
          return (value + 15U) & ~std::uint64_t{15U};
        };
        require(initial_mla.physical_outputs.size() > 1U && initial_mla.logical_outputs.size() > 1U,
        ...
        const auto& terminal_mla = real_facts[mla_indices[1]].mla_compiled->runtime_contract;
        require(terminal_mla.frame_arena_role == FrameArenaRole::Allocate &&
                    terminal_mla.frame_arena_storage_domain == sc::ArenaStorageDomain::Dms &&
        ...
        require(
            (terminal_scalar_bytes == 2U || terminal_scalar_bytes == 4U) &&
                ...
                terminal_mla.physical_outputs[1].size_bytes == 300U * terminal_row_pitch &&
                terminal_logits.stride_bytes ==
                    std::vector<std::int64_t>({static_cast<std::int64_t>(300U * terminal_row_pitch),
        ...
        struct PriorPhysicalPublication {
          ...
          const simaai::neat::pipeline_internal::sima::LogicalTensorStaticSpec* logical = nullptr;
          const simaai::neat::pipeline_internal::sima::PhysicalBufferStaticSpec* physical = nullptr;
        };
        ...
        for (std::size_t prior_index = 0U; prior_index < mla_indices[1]; ++prior_index) {
          ...
          for (std::size_t logical_index = 0U; logical_index < runtime->logical_outputs.size();
               ++logical_index) {
            const auto& logical = runtime->logical_outputs[logical_index];
            const auto physical =
                std::find_if(runtime->physical_outputs.begin(), runtime->physical_outputs.end(),
                             [&](const auto& candidate) {
                               return candidate.physical_index == logical.physical_index;
                             });
            if (physical != runtime->physical_outputs.end() && !logical.backend_name.empty()) {
              prior_publications.push_back(
                  PriorPhysicalPublication{prior_index, logical_index, &logical, &*physical});
            }
          }
        }
        require(terminal_mla.input_bindings.size() == terminal_mla.physical_inputs.size(),
        ...
        for (std::size_t index = 0; index < terminal_mla.physical_inputs.size(); ++index) {
          ...
          const auto publication = std::find_if(
              prior_publications.begin(), prior_publications.end(), [&](const auto& candidate) {
                const auto publication_index =
                    static_cast<std::size_t>(&candidate - prior_publications.data());
          ...
          const auto publication_index =
              static_cast<std::size_t>(&*publication - prior_publications.data());
          ...
          require(input.physical_index == static_cast<int>(index) &&
                      input.source_physical_index == 0 && input.source_byte_offset >= 0 &&
        ...
        require(std::count(producer_stage_used.begin(), producer_stage_used.end(), true) > 1,
        ...
        require(occurrences("model-path=") == 0U && occurrences("batch-size=") == 0U &&
        ...
        for (std::size_t index = 0; index < a65_contracts.size(); ++index) {
          ...
          require(contract.at("schema") == "sima.neat.direct-tvm-lane" &&
                      contract.at("version") == 2 && contract.at("storage_domain") == "cma" &&
                      contract.at("allocation_provenance") == "core_allocated" &&
                      (contract.at("required_device_access").get<std::uint32_t>() & 1U) != 0U &&
      ...
      if (const char* yolo_package = std::getenv("SIMANEAT_EXACT_YOLOV8_029DDB60_PACKAGE");
        ...
        ModelPack yolo_model(yolo_package, "application/vnd.simaai.tensor", "FP32", 3, 640, 640, 3,
                             false, {}, {}, {}, PipelineType::QuantTess, "decoder", 4, 4, 0, -1, {},
                             {}, terminal_policy,
                             /*cleanup_extracted_model_data=*/false);
        const auto full_digest = yolo_model.execution_plan_digest();
        const auto full_proof = yolo_model.execution_admission().proof;
        ...
        yolo_model.set_model_managed_stage_facts(flags, {ExecutionStageKind::BoxDecode});
        ...
        require(yolo_model.execution_plan_digest() == full_digest,
                "terminal selection changed the exact YOLO full-plan digest");
        require_proof_unchanged(full_proof, yolo_model.execution_admission().proof);
        require(yolo_model.infer_block().size() == 1U,
                "exact YOLO infer region must contain only the terminal MLA");

        const auto selected_facts = yolo_model.stage_facts_for_model_stage(ModelStage::Full);
        const simaai::neat::CompiledRuntimeContract* terminal_mla = nullptr;
        for (const auto& fact : selected_facts) {
          if (fact.mla_compiled.has_value()) {
            terminal_mla = &fact.mla_compiled->runtime_contract;
        ...
        const std::array<std::int64_t, 6U> expected_offsets{0,      409600,  512000,
                                                            537600, 1049600, 1177600};
        const std::array<std::uint64_t, 6U> expected_spans{409600U, 102400U, 25600U,
        ...
        for (std::size_t index = 0U; index < expected_offsets.size(); ++index) {
          ...
          require(logical.byte_offset == expected_offsets[index] &&
                      logical.size_bytes == expected_spans[index] && logical.physical_index == 0,
      ...
      if (const char* six_ofm_package = std::getenv("SIMANEAT_EXACT_YOLOV8N_SIX_REAL_OFM_PACKAGE");
          six_ofm_package != nullptr && *six_ofm_package != '\0') {
        simaai::neat::internal::InferenceTerminalPolicy terminal_policy;
        terminal_policy.mla_only = true;
        ModelPack six_ofm_model(six_ofm_package, "application/vnd.simaai.tensor", "FP32", 3, 640,
                                640, 3, false, {}, {}, {}, PipelineType::Quant, "decoder", 4, 4, 0,
                                -1, {}, {}, terminal_policy,
                                /*cleanup_extracted_model_data=*/false);
        ...
        flags.boxdecode_selected = true;
        flags.terminal_consumer_owns_tensor_tail = true;
        six_ofm_model.set_model_managed_stage_facts(flags, {ExecutionStageKind::BoxDecode});

        const auto selected_facts = six_ofm_model.stage_facts_for_model_stage(ModelStage::Full);
        ...
        require(terminal_mla != nullptr, "six-real-OFM selected route must retain terminal MLA");

        const auto& runtime = terminal_mla->runtime_contract;
        const std::array<std::int64_t, 6U> expected_offsets{0,      409600,  512000,
                                                            540672, 1052672, 1183744};
        const std::array<std::uint64_t, 6U> expected_extents{409600U, 102400U, 25600U,
                                                             512000U, 128000U, 32000U};
        require(runtime.frame_arena_storage_domain == sc::ArenaStorageDomain::Dms &&
                ...
                "(actual_bytes=" +
                    std::to_string(runtime.frame_arena_size_bytes) + ")");
        require(runtime.physical_outputs.size() == expected_offsets.size() &&
                    terminal_mla->dispatcher_physical_outputs.size() == expected_offsets.size() &&
        ...
        for (std::size_t index = 0U; index < expected_offsets.size(); ++index) {
          ...
          require(physical.physical_index == static_cast<int>(index) &&
                      ...
                      logical.byte_offset == 0 && logical.size_bytes == expected_extents[index],
                  "six-real-OFM port/view geometry changed at index " + std::to_string(index));
        }
      }
    }));
