...
RUN_TEST(
    "unit_error_code_taxonomy_test", ([] {
      ...
      // build.plugin_missing: an unknown element should identify the missing plugin.
      {
        Graph parse_fail;
        ...
        require_neat_error([&]() { (void)parse_fail.build(); }, error_codes::kPluginMissing,
                           "build.plugin_missing", "required GStreamer element is not installed");
      }
    }));
