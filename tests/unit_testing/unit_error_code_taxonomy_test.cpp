...
RUN_TEST("unit_error_code_taxonomy_test", ([] {
           ...
           // misconfig.pipeline_shape: build(input) without Input() must fail before runtime.
           {
             ...
             require_session_error([&]() { (void)missing_input.build(seed, RunMode::Sync); },
           ...
           // misconfig.caps: invalid caps_override should be rejected with the offending value.
           {
             ...
             require_session_error([&]() { (void)invalid_caps.build(seed, RunMode::Sync); },
           ...
           // misconfig.input_shape: malformed input tensor should carry input-shape taxonomy.
           {
             ...
             require_session_error([&]() { (void)bad_tensor_session.build(bad, RunMode::Sync); },
                                   error_codes::kInputShape, "misconfig.input_shape",
                                   "input tensor shape/spec inference failed");
           }
