...
simaai::neat::Sample run_canonical_model_sample(const cv::Mat& img_bgr, simaai::neat::Model& model,
  ...
  auto run_once_async = [&](int pull_timeout_ms) -> simaai::neat::Sample {
    ...
    if (std::getenv("SIMA_E2E_INPUTSTREAM_METRICS") != nullptr) {
      async_run_opt.enable_metrics = true;
    }
    ...
    if (std::getenv("SIMA_E2E_REPORT") != nullptr) {
      simaai::neat::RunReportOptions ropt;
      // Default ropt already enables stage_timings, element_timings, boundaries,
      // flow_stats, queue_depth, num_buffers, run_stats, input_stats.
...
int main(int argc, char** argv) {
  ...
  (void)argc;
  (void)argv;
  return skip_long_test("OpenCV required for yolov8_variant_route_matrix_test");
#else
  try {
    ...
    std::cout << "[OK] yolov8_variant_route_matrix_test models=" << packs.size() << "\n";
    return 0;
