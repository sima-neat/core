...
static simaai::neat::Model model_from_format(const std::string& tar_gz, int width, int height,
  ...
  auto color_format_from_string = [](const std::string& in) -> simaai::neat::PreprocessColorFormat {
    if (in == "RGB")
      return simaai::neat::PreprocessColorFormat::RGB;
    if (in == "BGR")
      return simaai::neat::PreprocessColorFormat::BGR;
    if (in == "GRAY" || in == "GRAY8")
      return simaai::neat::PreprocessColorFormat::GRAY8;
    if (in == "NV12")
      return simaai::neat::PreprocessColorFormat::NV12;
    if (in == "I420")
      return simaai::neat::PreprocessColorFormat::I420;
    return simaai::neat::PreprocessColorFormat::Auto;
  };
  simaai::neat::Model::Options opt;
  opt.preprocess.kind = simaai::neat::InputKind::Image;
  opt.preprocess.enable = simaai::neat::AutoFlag::On;
  opt.preprocess.color_convert.input_format = color_format_from_string(format);
...
static simaai::neat::Tensor require_tensor(const simaai::neat::Sample& out,
  ...
  auto tensors = simaai::neat::tensors_from_sample(out, /*require_nonempty=*/false);
  if (tensors.empty()) {
...
static simaai::neat::Tensor require_tensor(const simaai::neat::TensorList& outs,
                                           const std::string& label) {
  if (outs.empty()) {
    throw std::runtime_error(label + ": expected tensor output");
  }
...
int main(int argc, char** argv) {
  ...
  try {
    ...
    // Terminal policy validation checks.
    {
      ...
      bad_opt.preprocess.kind = simaai::neat::InputKind::Image;
      bad_opt.preprocess.enable = simaai::neat::AutoFlag::On;
      bad_opt.preprocess.color_convert.input_format = simaai::neat::PreprocessColorFormat::RGB;
    ...
    }
    {
      ...
      idx_opt.preprocess.kind = simaai::neat::InputKind::Image;
      idx_opt.preprocess.enable = simaai::neat::AutoFlag::On;
      idx_opt.preprocess.color_convert.input_format = simaai::neat::PreprocessColorFormat::RGB;
    ...
    }
    {
      ...
      mla_opt.preprocess.kind = simaai::neat::InputKind::Image;
      mla_opt.preprocess.enable = simaai::neat::AutoFlag::On;
