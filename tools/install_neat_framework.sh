...
# - Install NEAT runtime .deb packages on Modalix board, natively in the ROS2
#   SDK, or into the eLxr SDK sysroot.
#
# Behavior:
...
detect_env_mode() {
  ...
  # /etc/buildinfo. An unrecognized SDK release file is still rejected above.
  echo "modalix-board"
}
...
read_sdk_platform_version() {
  ...
  local value
  value="$(read_metadata_field "${release_file}" "SDK Version")"
  printf '%s\n' "${value%%_*}"
}

read_ros2_sdk_platform_version() {
  read_metadata_field "$1" "Platform Version"
}
