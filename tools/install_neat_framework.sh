...
B4593_PREFLIGHT_COMPLETE=0
PLATFORM_GUARD_SNAPSHOT_COMPLETE=0
B4593_KERNEL_PACKAGE="linux-image-6.18.3-modalix"
B4593_KERNEL_PACKAGE_VERSION="6.18.3-4593"
B4593_KERNEL_RELEASE="6.18.3-modalix"
B4593_KERNEL_BUILD_MARKER="#4593"
B4593_LIBCAMERA_VERSION="2.1.3+neat2"
B4593_LIBCAMERA_PLATFORM_COMPAT_VERSION="2.1.3~pre4593"
B4593_LIBCAMERA_CAPABILITY_NAME="simaai-libcamera-dmabuf-abi"
B4593_LIBCAMERA_CAPABILITY_VERSION="1"
B4593_MEMORY_VERSION="2.1.1-0neat5"
B4593_MEMORY_PLATFORM_COMPAT_VERSION="2.1.1~pre4593"
B4593_MEMORY_CAPABILITY_NAME="simaai-memory-dmabuf-export-abi"
B4593_MEMORY_CAPABILITY_VERSION="1"
...
load_b4593_platform_package_contract() {
  local manifest_path
  local -a values=()
  manifest_path="$(resolve_package_manifest_path)"
...
PY
  ...
  B4593_KERNEL_PACKAGE="${values[0]}"
  B4593_KERNEL_PACKAGE_VERSION="${values[1]}"
  B4593_KERNEL_RELEASE="${values[2]}"
  B4593_KERNEL_BUILD_MARKER="${values[3]}"
  B4593_LIBCAMERA_VERSION="${values[4]}"
  B4593_LIBCAMERA_PLATFORM_COMPAT_VERSION="${values[5]}"
  B4593_LIBCAMERA_CAPABILITY_NAME="${values[6]}"
  B4593_LIBCAMERA_CAPABILITY_VERSION="${values[7]}"
  B4593_MEMORY_VERSION="${values[8]}"
  B4593_MEMORY_PLATFORM_COMPAT_VERSION="${values[9]}"
  B4593_MEMORY_CAPABILITY_NAME="${values[10]}"
  B4593_MEMORY_CAPABILITY_VERSION="${values[11]}"
}

verify_b4593_running_kernel() {
  local installed_version installed_arch running_release version_text

  if ! deb_package_is_installed "${B4593_KERNEL_PACKAGE}"; then
    echo "Required B4593 kernel package is not installed: ${B4593_KERNEL_PACKAGE}=${B4593_KERNEL_PACKAGE_VERSION}." >&2
    return 1
  fi
  installed_version="$(deb_package_installed_version "${B4593_KERNEL_PACKAGE}")"
  if [[ "${installed_version}" != "${B4593_KERNEL_PACKAGE_VERSION}" ]]; then
    echo "Wrong B4593 kernel package version: installed ${B4593_KERNEL_PACKAGE}=${installed_version:-<missing>}, expected ${B4593_KERNEL_PACKAGE_VERSION}." >&2
    return 1
  fi
  installed_arch="$(dpkg-query -W -f='${Architecture}' "${B4593_KERNEL_PACKAGE}" 2>/dev/null || true)"
  ...
  if [[ "${running_release}" != "${B4593_KERNEL_RELEASE}" ]]; then
    echo "The installed B4593 kernel is not running: uname -r=${running_release}, expected ${B4593_KERNEL_RELEASE}. Reboot into ${B4593_KERNEL_PACKAGE_VERSION} before installing Neat." >&2
    return 1
  fi
  version_text="$(cat /proc/version 2>/dev/null || true)"
  if [[ " ${version_text} " != *" ${B4593_KERNEL_BUILD_MARKER} "* ]]; then
    echo "Running kernel ${running_release} does not report B4593 build marker ${B4593_KERNEL_BUILD_MARKER}." >&2
    return 1
  fi

  log "Verified running kernel ${running_release} (${B4593_KERNEL_PACKAGE_VERSION}, ${B4593_KERNEL_BUILD_MARKER})."
}
...
collect_local_simaai_memory_debs() {
  ...
  if [[ "${SIMAAI_MEMORY_PLATFORM_COMPAT_VERSION}" != "${B4593_MEMORY_PLATFORM_COMPAT_VERSION}" ]]; then
    echo "The installed/candidate Palette requires simaai-memory-lib=${SIMAAI_MEMORY_PLATFORM_COMPAT_VERSION}, but this bundle is restricted to B4593 compatibility ${B4593_MEMORY_PLATFORM_COMPAT_VERSION}." >&2
    return 1
  fi
  ...
  if [[ "${SIMAAI_MEMORY_ACTUAL_VERSION}" != "${B4593_MEMORY_VERSION}" ]]; then
    echo "Bundled simaai-memory-lib has version ${SIMAAI_MEMORY_ACTUAL_VERSION}; expected reviewed B4593 override ${B4593_MEMORY_VERSION}." >&2
    return 1
  fi
  ...
  capability_version="$(exact_dependency_version_from_relations \
    "${B4593_MEMORY_CAPABILITY_NAME}" "${B4593_MEMORY_CAPABILITY_VERSION}" \
    <<<"${provides}" || true)"
  if [[ "${capability_version}" != "${B4593_MEMORY_CAPABILITY_VERSION}" ]]; then
    echo "Bundled simaai-memory-lib=${SIMAAI_MEMORY_ACTUAL_VERSION} must provide ${B4593_MEMORY_CAPABILITY_NAME} (= ${B4593_MEMORY_CAPABILITY_VERSION}); got ${provides:-<none>}." >&2
    return 1
  fi
...
collect_local_libcamera_debs() {
  ...
  if [[ "${compatible}" != "${B4593_LIBCAMERA_PLATFORM_COMPAT_VERSION}" ]]; then
    echo "The installed/candidate Palette requires libcamera=${compatible}, but this bundle is restricted to B4593 compatibility ${B4593_LIBCAMERA_PLATFORM_COMPAT_VERSION}." >&2
    return 1
  fi
  if [[ "$(palette_required_package_version libcamera-tools)" != "${B4593_LIBCAMERA_PLATFORM_COMPAT_VERSION}" ]]; then
    echo "The B4593 Palette must require libcamera-tools=${B4593_LIBCAMERA_PLATFORM_COMPAT_VERSION}." >&2
    return 1
  fi
  ...
  for deb in "${runtime_deb}" "${dev_deb}" "${tools_deb}"; do
    ...
    arch="$(dpkg-deb -f "${deb}" Architecture)"
    if [[ "${version}" != "${B4593_LIBCAMERA_VERSION}" ]]; then
      echo "Bundled ${package} has version ${version:-<missing>}; expected reviewed B4593 override ${B4593_LIBCAMERA_VERSION}." >&2
      return 1
    fi
    if [[ "${arch}" != "${board_arch}" ]]; then
      echo "Bundled ${package} has architecture ${arch}; board architecture is ${board_arch}." >&2
    ...
    compatible="$(exact_dependency_version_from_relations \
      "${package}" "${B4593_LIBCAMERA_PLATFORM_COMPAT_VERSION}" \
      <<<"${provides}" || true)"
    if [[ "${compatible}" != "${B4593_LIBCAMERA_PLATFORM_COMPAT_VERSION}" ]]; then
      echo "Bundled ${package}=${version} must provide ${package} (= ${B4593_LIBCAMERA_PLATFORM_COMPAT_VERSION}); got ${provides:-<none>}." >&2
      return 1
    fi
  done
  ...
  capability="$(exact_dependency_version_from_relations \
    "${B4593_LIBCAMERA_CAPABILITY_NAME}" "${B4593_LIBCAMERA_CAPABILITY_VERSION}" \
    <<<"${provides}" || true)"
  if [[ "${capability}" != "${B4593_LIBCAMERA_CAPABILITY_VERSION}" ]]; then
    echo "Bundled libcamera=${B4593_LIBCAMERA_VERSION} must provide ${B4593_LIBCAMERA_CAPABILITY_NAME} (= ${B4593_LIBCAMERA_CAPABILITY_VERSION}); got ${provides:-<none>}." >&2
    return 1
  fi
  ...
  for deb in "${dev_deb}" "${tools_deb}"; do
    ...
    if [[ "${version}" != "${B4593_LIBCAMERA_VERSION}" ]]; then
      echo "Bundled ${package} must depend on libcamera (= ${B4593_LIBCAMERA_VERSION}); got ${depends:-<none>}." >&2
      return 1
    fi
  done
  ...
  LIBCAMERA_ACTUAL_VERSION="${B4593_LIBCAMERA_VERSION}"
...
validate_local_libcamera_payload() {
  ...
  log "Validated bundled libcamera=${LIBCAMERA_ACTUAL_VERSION} (provides ${B4593_LIBCAMERA_PLATFORM_COMPAT_VERSION} and ${B4593_LIBCAMERA_CAPABILITY_NAME}=${B4593_LIBCAMERA_CAPABILITY_VERSION}) payload sha256=${LIBCAMERA_PAYLOAD_SHA256} build-id=${LIBCAMERA_PAYLOAD_BUILD_ID}"
}
...
verify_installed_simaai_memory_payload() {
  ...
  if ! relation_field_provides_exact_version \
      "${provides}" "${B4593_MEMORY_CAPABILITY_NAME}" \
      "${B4593_MEMORY_CAPABILITY_VERSION}"; then
    echo "Installed simaai-memory-lib is missing ${B4593_MEMORY_CAPABILITY_NAME} (= ${B4593_MEMORY_CAPABILITY_VERSION})." >&2
    return 1
  fi
...
verify_installed_libcamera_payload() {
  ...
  if ! relation_field_provides_exact_version \
      "${provides}" "${B4593_LIBCAMERA_CAPABILITY_NAME}" \
      "${B4593_LIBCAMERA_CAPABILITY_VERSION}"; then
    echo "Installed libcamera is missing ${B4593_LIBCAMERA_CAPABILITY_NAME} (= ${B4593_LIBCAMERA_CAPABILITY_VERSION})." >&2
    return 1
  fi
...
preflight_b4593_board_install() {
  if [[ "${B4593_PREFLIGHT_COMPLETE:-0}" -eq 1 ]]; then
    return 0
  fi
  if [[ "${NEAT_INSTALLER_SKIP_PLATFORM_CHECK}" == "ON" ]]; then
    log "NEAT_INSTALLER_SKIP_PLATFORM_CHECK=ON; skipping the B4593 kernel and override package preflight."
    B4593_PREFLIGHT_COMPLETE=1
    return 0
  fi

  load_b4593_platform_package_contract || return 1
  verify_b4593_running_kernel || return 1
  collect_local_simaai_memory_debs || return 1
  validate_local_simaai_memory_payload || return 1
...
simaai-memory-lib	${B4593_MEMORY_VERSION}
simaai-memory-lib-dev	${B4593_MEMORY_VERSION}
libcamera	${B4593_LIBCAMERA_VERSION}
libcamera-dev	${B4593_LIBCAMERA_VERSION}
libcamera-tools	${B4593_LIBCAMERA_VERSION}
EOF
  return 1
}
...
install_local_b4593_override_transaction() {
  local simulation_log
  local -a apt_args=(apt-get install -y --reinstall --no-remove)

  preflight_b4593_board_install || return 1
  if [[ "${NEAT_INSTALLER_SKIP_PLATFORM_CHECK}" == "ON" ]]; then
  ...
  simulation_log="$(mktemp /tmp/sima-neat-b4593-apt-simulation-XXXXXX)"
  INSTALLER_TMP_DIRS+=("${simulation_log}")
...
native_modalix_repair_is_required() {
  ...
  for package in \
    ...
    if [[ "${package}" == libcamera* &&
          "${version}" == "${B4593_LIBCAMERA_VERSION}" ]]; then
      continue
    fi
...
install_debs_on_board() {
  prepare_debs_for_board_install
  if ! preflight_b4593_board_install; then
  ...
  stop_board_runtime_before_install
  if ! install_local_b4593_override_transaction; then
