...
A `Graph` is the assembly boundary; [`Node`](/reference/programming-model/node) is the building block.
...
For the detailed rules around pre-built groups and boundary nodes, see [Node → Pre-built node groups](/reference/programming-model/node#pre-built-node-groups) and [Node → Boundary nodes](/reference/programming-model/node#boundary-nodes).
