...
| `allow_cpu_fallback` | `true` | If `true`, use Neat's private adaptive bridge into SiMaAI memory. If `false`, require camera/device zero-copy support. |
...
`allow_cpu_fallback = false` requests strict camera/device zero-copy. The build fails unless the installed `libcamerasrc` exposes the required SiMaAI zero-copy properties and the memory library can export the camera allocation as a DMA-BUF.

`allow_cpu_fallback = true` inserts Neat's private adaptive camera memory bridge. The bridge passes through SiMaAI/EV74 buffers when upstream already provides them. Otherwise, it copies the camera frame into a pooled SiMaAI buffer and attaches the metadata downstream stages expect.
