...
| `allow_cpu_fallback` | `false` | If `false`, require camera/device zero-copy support. If `true`, use Neat's private adaptive bridge into SiMaAI memory. |
...
`allow_cpu_fallback = false` requests strict camera/device zero-copy. The build fails if the installed `libcamerasrc` does not expose the SiMaAI zero-copy properties Neat needs.
