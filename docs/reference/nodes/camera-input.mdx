...
| `capture_buffer_count` | `0` | ISP-output/libcamera buffer count. `0` uses the camera default; applications that retain frames may request up to `32`. |
...
before the live queue. The bridge proposes Neat's SiMaAI allocator through
GStreamer's downstream `GST_QUERY_ALLOCATION`. `libcamerasrc` selects that
allocator, derives plane sizes and strides from the validated camera
configuration, allocates the packed frames, and imports their DMA-BUFs into the
ISP capture queue. The bridge verifies and forwards those buffers without a
copy in strict mode.

Set `capture_buffer_count` based on the downstream owner. A temporal encoder or
deep asynchronous ML graph may need 32 buffers; a low-latency CV graph may use
fewer and drop stale frames. This setting controls the ISP-output/application
cycle only. It does not enlarge the kernel's private CSI-to-ISP RAW transit
ring, and it is independent of the GStreamer `queue_depth` drop policy.

For the B4593 integration branch, the kernel, libcamera, and memory overrides
form one package contract. Contributors should follow the
[B4593 DMA-BUF package contract](/develop-apps/contribute/b4586-dmabuf-package-contract/)
instead of installing individual override DEBs.
