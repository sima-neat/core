...
| `compute_version.sh` | Compute the framework's package version string from `deps/manifest.json` `package-version`, plus git metadata for branch builds. Used by CI and packaging. |
