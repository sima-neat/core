...
- The Neat Library C++ ABI is now 5 and the shared-library SONAME is `libsima_neat.so.5`. Tensors now carry feature-extractor semantic metadata, public GenAI request/result types carry ASR task, language, and probe metadata, and `GraphLinkOptions` contains realtime admission limits. Rebuild C++ applications and plugins and install matching Core runtime and development packages.
