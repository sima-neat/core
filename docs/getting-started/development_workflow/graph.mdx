...
A `Graph` is the assembly boundary; [`Node`](/getting-started/development_workflow/node) is the building block.
...
For the detailed rules around pre-built groups and boundary nodes, see [Node → Pre-built node groups](/getting-started/development_workflow/node#pre-built-node-groups) and [Node → Boundary nodes](/getting-started/development_workflow/node#boundary-nodes).
