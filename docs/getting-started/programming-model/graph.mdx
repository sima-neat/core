...
A `Graph` is the assembly boundary; [`Node`](/getting-started/programming-model/node) is the building block.
...
For the detailed rules around pre-built groups and boundary nodes, see [Node → Pre-built node groups](/getting-started/programming-model/node#pre-built-node-groups) and [Node → Boundary nodes](/getting-started/programming-model/node#boundary-nodes).
