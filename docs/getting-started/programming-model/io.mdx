...
Key options:
- `device` — V4L2 device path (default: `/dev/video0`; must not be empty)
- `media_type` / `format` / `width` / `height` — caps enforcement (capsfilter appended when `media_type` is non-empty and both `width` and `height` are positive)
- `fps_n` / `fps_d` — framerate numerator/denominator (capsfilter uses `framerate=fps_n/fps_d`; default 0/1 omits the field — recommended for Modalix where the ISP may advertise a non-standard rate)
- `io_mode` — V4L2 I/O method (`"mmap"`, `"dmabuf"`, `"dmabuf-import"`, etc.)
- `num_buffers` — V4L2 buffer count; -1 (default) omits the property, letting GStreamer use its built-in default (continuous capture)
...
- Source is file/RTSP: use input groups from `nodes::groups`.
- Source is V4L2 camera (USB, MIPI post-ISP): use `nodes::V4L2Input`.
...
- [MIPI Camera Input](/tutorials/v2/019-mipi-camera-input)
- [MIPI Camera to UDP H264](/tutorials/v2/020-mipi-camera-to-udp)
