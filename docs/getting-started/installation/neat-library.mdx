...
[Neat Development Environment](/getting-started/installation/dev-environment/).

## Install PyNeat in a Custom Virtual Environment

The installer provisions the `~/pyneat` virtual environment for you. If you need
a custom Python environment, install the PyNeat wheel into a separate venv,
conda environment, or other Python 3.11 setup.

1. Create and activate your target Python 3.11 environment:
...
2. Install the PyNeat version that matches your Neat Library version:

   ```bash
   pip install pyneat==0.1.0
   ```

You can run this command from a compatible Neat Development Environment or on
the DevKit. `pip` retrieves the matching PyNeat wheel and installs it into the
active environment.

The published wheel targets **CPython 3.11** on **`linux_aarch64`**, so install
it into a matching environment. For supported Neat Library and SDK combinations,
see the [Compatibility Guide](/getting-started/compatibility/).
...
Components
  ...
  Model Compiler        2.0.0.neat+main-1ebbc39 run activate-model-sdk to activate
