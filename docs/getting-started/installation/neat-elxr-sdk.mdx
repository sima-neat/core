...
The Neat SDK is built on top of the [open source SiMa.ai eLxr SDK](https://github.com/SiMa-ai/swsoc-simaai-elxr-doc), with workflow-focused additions:
...
The Model SDK Extension adds tools for quantizing and compiling ONNX models so they can run on SiMa.ai's MLA (Machine Learning Accelerator). To install it, opt in when prompted during Neat SDK installation.
...
│                                                                                                                       │
│ The Model SDK Extension lets you quantize and compile models so they can run on SiMa.ai hardware accelerated.            │
...
```text
Build a C++ app doing image classification with RESNET50 using SiMa.ai Neat Framework, run it on the devkit until you get the result.
```
