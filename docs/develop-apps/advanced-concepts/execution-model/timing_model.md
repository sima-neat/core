...
The framework does not promise FIFO across multiple inputs streams unless you explicitly ask for it via `RunPreset` or per-stream queues — see [Tutorial 010: feed a multi-input model](/tutorials/feed-multi-input-model).
