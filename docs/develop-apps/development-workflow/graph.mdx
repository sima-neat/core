...
| Connect live graph fragments | `GraphLinkOptions` with a realtime-by-stream policy | Use `RealtimeLatestByStream` for freshness or `RealtimeEveryFrameByStream` for bounded per-stream backpressure. |
...
Use [`GraphLinkOptions`](/reference/{lsa}/structs/simaai-neat-graphlinkoptions) when the connection between fragments needs a runtime policy or explicit raw-frame admission limits.
...
Fusion is an internal compiler decision, not a build mode. Ordinary `build()` keeps eligible private source branches, their realtime mux, and the model consumer in one GStreamer pipeline, avoiding an appsink/appsrc device-memory handoff. A realtime-by-stream link whose topology cannot be fused is rejected; Core does not fall back to segmented appsrc transport.

Set `GraphOptions::async_queue_depth` on the top-level graph to overlap model stages. A positive value inserts bounded, non-leaky global queues before the CVU, MLA, and decode stages, but not before the terminal output. `GraphLinkOptions::max_inflight_per_stream` is carried through to the fused mux for each realtime link, so it can stay at `1` for freshness without a private environment switch. Leave `async_queue_depth` at `0` to preserve the single-chain consumer path.

`GraphLinkOptions` provides all five fields below in C++ and Python and is passed to ordinary `connect(...)`. The inflight fields have no effect while `policy` is `Default`; automatic live fan-in promotion uses the framework admission defaults.
