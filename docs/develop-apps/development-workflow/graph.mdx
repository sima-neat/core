...
Use [`GraphLinkOptions`](/reference/{lsa}/structs/simaai-neat-graphlinkoptions) when the connection between fragments needs a runtime policy.
...
```cpp
auto run = app.build(simaai::neat::fuse_realtime_source_branches, run_options);
```
...
`GraphLinkOptions` has these realtime-link fields:
