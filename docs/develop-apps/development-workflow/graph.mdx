...
Use the default link policy when every frame must be preserved and backpressure is the correct behavior. Use realtime latest-by-stream when a live graph should stay fresh instead of collecting old frames. Set `queue_depth` only when you have a reason to bound pending samples on that link differently from the default.
...
| `max_inflight_per_stream` | A realtime latest-by-stream link carries raw decoder-backed samples and needs more admitted downstream frames per stream. The field default is `-1`; Core uses `4`. Fused source builds apply the resolved value to the corresponding mux stream. |
| `max_inflight_total` | A realtime latest-by-stream link needs a hard global cap across streams. The field default is `-1`; without an env override, Core derives `min(max_inflight_per_stream * stream_count, 8)`. |
