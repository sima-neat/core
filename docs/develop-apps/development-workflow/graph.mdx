...
:::caution Rebuild C++ applications after upgrading

The admission fields are members of `GraphLinkOptions`, so this version changes its C++ binary layout. Rebuild C++ applications and plugins against the matching Core package instead of mixing old binaries with the new library.

When migrating from an earlier preview, replace `RealtimeMuxByStream` with `GraphLinkOptions`, replace `RealtimeEveryFrameByStream` with `RealtimeLatestByStream`, and use ordinary `connect()` and `build()`. Rebuild and save any graph that contains the removed `realtime_every_frame_by_stream` policy.
