...
| Connect live graph fragments | `RealtimeGraphLinkOptions` with a realtime-by-stream policy | Use `RealtimeLatestByStream` for freshness or `RealtimeEveryFrameByStream` for bounded per-stream backpressure. |
...
Use [`GraphLinkOptions`](/reference/{lsa}/structs/simaai-neat-graphlinkoptions) when the connection between fragments needs a runtime policy. Use `RealtimeGraphLinkOptions` when the link also needs explicit raw-frame admission limits. The separate type preserves the released `GraphLinkOptions` binary layout.

For live multistream fan-in, `RealtimeLatestByStream` keeps the latest sample per `Sample::stream_id` and schedules ready streams fairly downstream. `RealtimeEveryFrameByStream` instead keeps one pending sample per stream and blocks only the producer for that stream until the mux consumes it. The every-frame mode reuses the existing depth-one ingress queue; it does not add another raw-frame queue. If a source does not stamp `stream_id`, you can stamp a stable id on the link.
...
To preserve every decoded frame with the same fused C++ mux, select the every-frame policy instead:
...
Use `RealtimeEveryFrameByStream` when a fused decoder source must preserve bursty frames with bounded backpressure. Use `RealtimeLatestByStream` when a live graph should stay fresh instead of collecting old frames. Live fan-in using the default policy is automatically promoted to latest-by-stream, so select every-frame explicitly when preservation is required. Set `queue_depth` only when you have a reason to bound pending samples on that link differently from the default.

For a high-channel-count live graph, explicitly select the fused source build so eligible private source branches stay with their fan-in and model consumer in one GStreamer pipeline:

```cpp
auto run = app.build_fused_realtime_sources(run_options);
```

```python
run = app.build_fused_realtime_source(run_options)
```

The distinct build entry point avoids an appsink/appsrc device-memory handoff without changing the behavior or binary layout of existing `RunOptions` callers. It replaces the private `SIMA_GRAPH_LEGACY_FUSED_REALTIME_INGRESS` switch. Eligible links using either realtime-by-stream policy are fused. Set `GraphOptions::async_queue_depth` on the top-level graph to overlap model stages. A positive value inserts bounded, non-leaky global queues before the CVU, MLA, and decode stages, but not before the terminal output. `RealtimeGraphLinkOptions::max_inflight_per_stream` is carried through to the fused mux for each link, so it can stay at `1` for freshness without `SIMA_LATEST_MUX_MAX_INFLIGHT_PER_STREAM`. Leave `async_queue_depth` at `0` to preserve the single-chain consumer path.

`GraphLinkOptions` provides the first three fields below. `RealtimeGraphLinkOptions` derives from it and adds the two admission fields. C++ accepts the extended type through `connect(...)` and also provides the explicit `connect_realtime(...)` spelling. Python keeps these fields together on `pyneat.GraphLinkOptions` because Python objects do not cross the C++ ABI boundary.

| Field | Use it when |
| --- | --- |
| `policy` | The link needs latest-frame freshness or bounded every-frame delivery per stream. |
| `queue_depth` | The link needs an explicit pending-sample bound. Leave the default until measurement shows the link is the bottleneck. |
| `stream_id` | The upstream fragment does not stamp `Sample::stream_id`, but this link represents one stable stream. |
| `max_inflight_per_stream` | A realtime-by-stream link carries raw decoder-backed samples and needs an explicit downstream admission cap per stream. The field default is `-1`; Core uses `4`. Fused source builds apply the resolved value to the corresponding mux stream. |
