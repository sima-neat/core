...
When `RunOptions::advanced.fuse_realtime_source_branches` keeps a live fan-in and its model consumer in one GStreamer pipeline, set `GraphOptions::async_queue_depth` on the top-level graph to overlap model stages. A positive value inserts bounded, non-leaky global queues before the CVU, MLA, and decode stages, but not before the terminal output. The per-stream admission limit remains controlled separately by `GraphLinkOptions::max_inflight_per_stream`, so it can stay at `1` for freshness. Leave `async_queue_depth` at `0` to preserve the single-chain fused path.
...
| `max_inflight_per_stream` | A realtime latest-by-stream link carries raw decoder-backed samples and needs more admitted downstream frames per stream. The field default is `-1`; without an env override, Core uses `4`. |
