...
| Connect live graph fragments | `RealtimeMuxByStream` with a realtime-by-stream policy | Use `RealtimeLatestByStream` for freshness or `RealtimeEveryFrameByStream` for bounded per-stream backpressure. |
...
Use [`GraphLinkOptions`](/reference/{lsa}/structs/simaai-neat-graphlinkoptions) when the connection between fragments needs a runtime policy. Use `RealtimeMuxByStream` when the link also needs explicit raw-frame admission limits. The separate type preserves the released `GraphLinkOptions` binary layout.
...
```python
link = pyneat.RealtimeMuxByStream()
...
Fusion is an internal compiler decision, not a build mode. Ordinary `build()` keeps eligible private source branches, their realtime mux, and the model consumer in one GStreamer pipeline, avoiding an appsink/appsrc device-memory handoff. Ineligible latest-by-stream topology uses the ordinary segmented runtime. An every-frame link whose topology cannot be fused is rejected with an actionable diagnostic because silently falling back would change its backpressure contract.

Set `GraphOptions::async_queue_depth` on the top-level graph to overlap model stages. A positive value inserts bounded, non-leaky global queues before the CVU, MLA, and decode stages, but not before the terminal output. `RealtimeMuxByStream::max_inflight_per_stream` is carried through to the fused mux for each link, so it can stay at `1` for freshness without a private environment switch. Leave `async_queue_depth` at `0` to preserve the single-chain consumer path.

`GraphLinkOptions` provides the first three fields below. `RealtimeMuxByStream` derives from it and adds the two admission fields. C++ accepts both types through `connect(...)`; no separate realtime connection method is needed. `pyneat.GraphLinkOptions` mirrors the released three fields; `pyneat.RealtimeMuxByStream` adds the two admission fields and is still passed to ordinary `connect(...)`.
