...
This version adds admission fields to `GraphLinkOptions`. The current Neat Library C++ ABI and shared-library SONAME are 4. Rebuild C++ applications and plugins against the matching Core package. Do not mix binaries built for `libsima_neat.so.3` with `libsima_neat.so.4`.
