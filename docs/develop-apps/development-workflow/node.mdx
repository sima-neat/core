...
Use `SimaDecode` for encoded streams that should be decoded by the native SiMa
decoder. It accepts H.264, JPEG, and MJPEG after the stream has already been
framed into complete encoded samples:

| Encoded input | Native decoder option | Typical upstream path |
| --- | --- | --- |
| H.264 access units | `SimaDecodeType::H264` | RTSP H.264 depacketize + parse, or another parsed H.264 source |
...
encoded.codec = simaai::neat::nodes::groups::RtspCodec::H264; // default

simaai::neat::nodes::groups::RtspDecodedInputOptions decoded;
decoded.url = "rtsp://camera.example/live";
decoded.codec = simaai::neat::nodes::groups::RtspCodec::MJPEG;
...
| Live RTSP encoded H.264 or MJPEG stream | `nodes::groups::RtspEncodedInput(...)` / `pyneat.groups.rtsp_encoded_input(...)` | `url`, `codec` when not H.264 | `latency_ms`, `tcp`, `drop_on_latency`, `buffer_mode`, RTP payload type |
| Live RTSP decoded H.264 or MJPEG stream | `nodes::groups::RtspDecodedInput(...)` / `pyneat.groups.rtsp_decoded_input(...)` | `url`, `codec` when not H.264 | `latency_ms`, `tcp`, `drop_on_latency`, `buffer_mode`, decoded width/height/FPS, `out_format`, `output_caps` |
