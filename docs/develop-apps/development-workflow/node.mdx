...
bitstreams are not supported. Encode grayscale sources as 8-bit 4:2:0.

`SimaDecodeOptions::raw_output` defaults to `true`. In that mode the decoder
emits native raw buffers, normally NV12 in SiMaAI memory, which is the intended
path before CVU preprocessing and MLA inference. Set `raw_output=false` only
when the next stage needs CPU-visible `video/x-raw(memory:SystemMemory)` buffers.
...
stream into decode and video-output graphs. `RtspDecodedInput(...)` appends
`SimaDecode(...)` and returns raw decoded video.

`source_fps` declares the source cadence. Set it when the RTSP caps do not carry
a valid frame rate; without either value, decoder startup fails. Neat does not
probe the RTSP URL for this value. For H.265, the value is propagated into the
parsed encoded caps and the decoder `dec-fps` setting. It does not request rate
...
std::vector<std::uint8_t> bytes = /* one parsed H.265 access unit */;
