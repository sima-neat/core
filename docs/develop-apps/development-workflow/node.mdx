...
equals `HEVC`. The same aliases are available through `RtspCodec` in C++ and
Python.
...
| Raw or encoded H.264/H.265 video output | `nodes::groups::VideoSender(...)` / `pyneat.groups.video_sender(...)` | `VideoSenderOptions::H264RtpUdpFromRaw(...)`, `H264RtpUdpFromEncoded()`, or `H265RtpUdpFromEncoded()` | `host`, `channel`, `video_port_base`, encoder/RTP options |
...
| Encoded H.265 input | `payload_type = Encoded`, `format = ENCODED`, and H.265 caps on the boundary and pushed `Sample` | The graph source owns and parses the stream through an RTSP or file group. |
...
identify the codec through caps, and select the matching decoder type. There is
no `FormatTag::H265`; use the generic `ENCODED` tag with `video/x-h265` caps.
Set the decoder width, height, and FPS to the elementary stream values:
...
iopt.format = simaai::neat::FormatTag::ENCODED;
iopt.caps_override =
...
iopt.format = pyneat.Format.ENCODED
iopt.caps_override = (
    "video/x-h265,parsed=true,stream-format=byte-stream,alignment=au"
)
