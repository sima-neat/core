...
decoder's native NV12 buffer. `RunOptions::output_memory` separately controls
whether a terminal output shares its backing buffer (`ZeroCopy`) or is copied
into framework-owned storage (`Owned`).
