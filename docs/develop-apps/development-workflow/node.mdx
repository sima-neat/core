...
| --- | --- |
| `true` (default) | Native decoded buffers, NV12 by default, without a software conversion stage. |
| `false` | Converted `video/x-raw(memory:SystemMemory)` output for CPU consumers. |

`SimaDecode` always requests `zero-copy-output=true`: the hardware decoder writes
into its DMA-BUF output pool, regardless of whether the next reader is the app,
CVU, MLA, or an encoder. Setting `raw_output=false` adds an explicit software
conversion stage; it does not change the decoder's allocation policy.
