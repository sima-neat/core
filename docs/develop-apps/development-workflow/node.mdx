...
| Live RTSP encoded H.264 or MJPEG stream | `nodes::groups::RtspEncodedInput(...)` / `pyneat.groups.rtsp_encoded_input(...)` | `url`, `decode_type` when not H.264 | `latency_ms`, `tcp`, `drop_on_latency`, `buffer_mode`, RTP payload type |
| Live RTSP decoded H.264 or MJPEG stream | `nodes::groups::RtspDecodedInput(...)` / `pyneat.groups.rtsp_decoded_input(...)` | `url`, `decode_type` when not H.264 | `latency_ms`, `tcp`, `drop_on_latency`, `buffer_mode`, decoded width/height/FPS, `out_format`, `output_caps` |
