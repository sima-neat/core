...
`source_fps` declares the source cadence. Neat does not probe the RTSP URL for
this value. For H.265, the value is propagated into the parsed encoded caps and
the decoder `dec-fps` setting. It does not request rate conversion. To change
the output cadence, enable `use_videorate` and set `video_rate_fps`.
