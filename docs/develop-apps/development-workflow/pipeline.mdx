...
| App consumes outputs without a transport copy | `OutputMemory::Auto` | Retains standard DMA-BUF outputs in sync and async runs; other storage follows the preset/mode policy. |
...
### Retain and release DMA-BUF outputs

Native hardware decode writes directly into a DMA-BUF pool. The default file-input
path preserves that storage: it does not insert a hidden `videoconvert` or request
SystemMemory. Device consumers import the allocation, and `Tensor::map(MapMode::Read)` maps
it for app reads without creating a pixel copy. A preprocessing operation can
produce a different result allocation; that is computation, not a transport copy.

With `OutputMemory::Auto` or `ZeroCopy`, retained DMA-BUF outputs keep their pool
loans. A `Mapping` keeps its loan even after you release the Tensor. Release both
when you finish reading so the producer can reuse its buffers. Queue pressure
does not switch DMA-BUF outputs to copies or silently turn `Block` into dropping;
explicit latest-frame/drop policies still apply.

Use `OutputMemory::Owned` or `clone()` when you deliberately need independent
storage. Explicit software conversion and `advanced.copy_input` remain copy
boundaries. `SIMA_OUTPUT_MEMORY_DEFAULT=owned` deliberately selects owned output
for `Auto`; it does not override an explicit per-run choice.
