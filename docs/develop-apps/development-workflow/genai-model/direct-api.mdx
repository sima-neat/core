...
`GenerationRequest.reasoning_effort` controls how much a gpt-oss model reasons.
Accepted values are `low`, `medium` and `high`; the default is `low`. Any other
value is rejected when the request is validated. This is independent of
`enable_thinking`: effort sets how much the model reasons, while the flag sets
whether that reasoning is returned to you.
