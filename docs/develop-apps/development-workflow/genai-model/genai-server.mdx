...
For gpt-oss models, `reasoning_effort` controls how much the model reasons.
Accepted values are `low`, `medium` and `high`; the default is `low`. Any other
value is rejected with HTTP 400. Effort is independent of `enable_thinking`,
which only controls whether the reasoning is returned:
...
`reasoning_effort` is also accepted inside `chat_template_kwargs`, which takes
precedence over the top-level field, as it does for `enable_thinking`:
