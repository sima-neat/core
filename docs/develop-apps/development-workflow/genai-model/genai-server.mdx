...
int main() {
  ...
  simaai::neat::genai::GenAIModelOptions llm_options;
  llm_options.max_kv_cache_slots = 4;
  server.add_model(
...
llm_options = neat.genai.GenAIModelOptions()
llm_options.max_kv_cache_slots = 4
server.add_model(
    "/media/nvme/llima/models/Qwen3-4B-Instruct-2507-GPTQ-a16w4",
    "llm",
...
Before listening, the server performs a silent warm-up inference for each
registered model so the first client request does not pay the full initialization
cost. Warm-up does not print generated text or performance statistics.
...
| `POST` | `/remove_cache` | Release one reusable prompt context. |
| `POST` | `/clear_caches` | Release all reusable prompt contexts for a served model. |
...
## Reuse prompt contexts

Add the same non-empty `cache_id` to each OpenAI- or Ollama-compatible request
that belongs to one prompt context:
...
curl http://<modalix-ip>:9998/v1/chat/completions \
  ...
  -d '{
    "model": "llm",
    "cache_id": "driver-assistant",
    "messages": [
      {"role": "system", "content": "<large infotainment and tool prompt>"},
...
The server keeps at most the number of contexts configured for that model with
`GenAIModelOptions.max_kv_cache_slots`. The default is one, and requests that
omit `cache_id` use a legacy default context that counts toward the same limit.
There is no automatic eviction. A request for a new ID at capacity returns HTTP
429 with error type `cache_capacity_error`.

Non-streaming responses include `cache_created` and
`usage.cached_prompt_tokens`. Streaming OpenAI and Ollama responses include
`cache_created` and `cached_prompt_tokens` with their other generation metrics.

Release a context when the application session ends:

```bash
curl http://<modalix-ip>:9998/remove_cache \
  -H "Content-Type: application/json" \
  -d '{"model":"llm","cache_id":"driver-assistant"}'
```

Release every context for one served model with:

```bash
curl http://<modalix-ip>:9998/clear_caches \
  -H "Content-Type: application/json" \
  -d '{"model":"llm"}'
```

The owning application can perform the same operations with
`server.remove_kv_cache(served_name, cache_id)`,
`server.clear_kv_caches(served_name)`, and
`server.kv_cache_count(served_name)`. Aliases registered with the same
`GenAIModel` share its context pool.
...
`model`, invalid cache ID, invalid tool configuration, an incompatible model
capability, or a missing multipart audio `file`. An unknown served model
returns HTTP 404. Exhausting a model's configured cache slots returns HTTP 429.
Before a response stream is established, other exceptions raised while parsing
a request or running a model return HTTP 500.
