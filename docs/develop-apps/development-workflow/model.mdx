...
### Explicit MLA input storage layout

Leave `input_storage_layouts` empty to use the MPK contract. If the model was
compiled with an input storage layout that the MPK does not distinguish, set the
layout using the exact public input name from `mpk.json`:

```cpp
simaai::neat::Model::Options options;
options.input_storage_layouts["input"] = simaai::neat::InputStorageLayout::HWC16;
simaai::neat::Model model(model_path, options);
```

```python
options = pyneat.ModelOptions()
options.input_storage_layouts = {"input": pyneat.InputStorageLayout.HWC16}
model = pyneat.Model(model_path, options)
```

`HWC` stores the logical channels consecutively. `HWC16` rounds each pixel's
channel storage up to a multiple of 16 bytes. For seven channels, Core retains
the logical seven-channel tensor and reserves sixteen INT8 or eight BF16 slots
per pixel.
This option describes the compiled MLA input. Application input tensors retain
their logical shape; Core adapts the model's input conversion output.

The adaptation requires an unambiguous Cast or Quantize path from the named
public input to a dense NHWC MLA input. Unknown names, unsupported paths, and
storage extents that contradict the compiled executable are rejected. Set the
option from the model's compilation settings; buffer size alone cannot identify
the layout.

This option extends `Model::Options` in the unreleased 0.4.0 API. Rebuild C++ applications and the Python wheel together with Core; older compiled consumers have a different options layout.
...
A default public `Input` directly connected to one model image ingress inherits
that ingress's image interpretation and format. You can still set the source's
format explicitly; the graph validates it against the model contract. Source
memory, dimensions, timestamps, and queue options remain unchanged. For multiple
or ambiguous consumers, provide explicit input options.

This interpretation applies at the Graph input boundary. Direct model calls still
require inputs that satisfy the model's declared image or tensor contract.
...
`push` waits for input-queue space. To submit without that wait, use C++ `runner.try_push` with a `Sample` or `TensorList`, or Python `runner.try_push_samples` / `runner.try_push_tensors`. Set `RunOptions.queue_depth` and `overflow_policy` before building the runner. When a bounded queue is full, `KeepLatest` replaces the oldest pending sample and returns `true`; `Block` and `DropIncoming` return `false`. Closed input also returns `false`. Keep a frame's paired tensors together in one TensorSet sample. Replacement does not cancel active inference, and the forwarding methods add no payload copy.
...
For a BoxDecode route, describe the synthetic input's source-image geometry so the benchmark can
map detections back from model coordinates:
...
Set `original_width` and `original_height` together. The benchmark still generates a model-shaped
synthetic tensor; these fields supply the source geometry and resize transform required by
BoxDecode metadata. If you omit them, the benchmark infers geometry from the resolved preprocess
and model input contracts. These per-run values take precedence over deprecated BoxDecode geometry
hints in `ModelOptions`. Existing `benchmark(num_samples)` calls keep the same behavior.
