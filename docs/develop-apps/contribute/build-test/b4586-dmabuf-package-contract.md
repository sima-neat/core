---
title: B4586 DMA-BUF Package Contract
description: Build and install the matching Core and Internals camera-stack branches on the B4586 kernel
sidebar_position: 5
slug: /develop-apps/contribute/b4586-dmabuf-package-contract
---

# B4586 DMA-BUF Package Contract

The `feature/b4586-dmabuf-zero-copy` integration is intentionally restricted to
the B1408/pre4586 Modalix package cohort. It combines the DMA-BUF kernel ABI with
...
| --- | --- |
| Running kernel | `linux-image-6.18.3-modalix=6.18.3-4586`, release `6.18.3-modalix`, build marker `#4586` |
| Libcamera override set | `libcamera`, `libcamera-dev`, and `libcamera-tools` at `2.1.3+neat1` |
| Libcamera Palette compatibility | Each package provides its `2.1.3~pre4586` identity |
...
| Memory Palette compatibility | Both packages provide their `2.1.1~pre4586` identity |
| Memory DMA-BUF capability | Runtime provides `simaai-memory-dmabuf-export-abi (= 1)` |

The pre-release repository is a source for the native B4586 cohort, not a
...
2. Confirm the B4586 kernel package is installed and is the running kernel.
...
development escape hatch. Do not use it for B4586 qualification evidence.
