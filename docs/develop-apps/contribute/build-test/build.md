...
For prebuilt package installation, see [Neat Library](/getting-started/neat-library/).
...
- Core and development packages (`*.deb`) are generated on Linux full builds unless `--no-dist` is used.
